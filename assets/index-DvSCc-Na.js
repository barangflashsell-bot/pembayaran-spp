...
`);z(`Laporan_Pembayaran_${new Date().toISOString().slice(0,10)}.csv`,a)}function re(e=25e4){let t=[`NIS`,`Nama Siswa`,`Kelas`,`Nama Orang Tua / Wali`,`No WhatsApp / HP`,`Nominal SPP (Rp)`],n=[[`2026011`,`Muhammad Rizky Pratama`,`VII-A`,`Bambang Pratama`,`081234567890`,e],[`2026012`,`Anisa Rahmawati`,`VII-B`,`Hendra Gunawan`,`085712345678`,e],[`2026013`,`Dimas Arya Saputra`,`VIII-A`,`Suryanto`,`087812345678`,e],[`2026014`,`Zahra Putri Kirana`,`IX-B`,`Wahyudi`,`089612345678`,e]];z(`Template_Data_Siswa.csv`,[t.map(B).join(`,`),...n.map(e=>e.map(B).join(`,`))].join(`\r
`))}function U(e,t){let n=[],r=``,i=!1;for(let a=0;a<e.length;a++){let o=e[a];o===`"`?i&&e[a+1]===`"`?(r+=`"`,a++):i=!i:o===t&&!i?(n.push(r.trim()),r=``):r+=o}return n.push(r.trim()),n}function ie(e){let t=e.split(/\r\n|\r|\n/)[0]||``,n=(t.match(/,/g)||[]).length,r=(t.match(/;/g)||[]).length,i=(t.match(/\t/g)||[]).length;return r>n&&r>i?`;`:i>n&&i>r?`	`:`,`}function ae(e,t=25e4){let n=e.replace(/^\uFEFF/,``).trim(),r=n.split(/\r\n|\r|\n/).filter(e=>e.trim().length>0);if(r.length<2)return{valid:[],errors:[`File kosong atau hanya berisi baris header.`],totalRows:0};let i=ie(n),a=U(r[0],i).map(e=>e.toLowerCase().replace(/[^a-z0-9]/g,``)),o=a.findIndex(e=>e===`nis`||e.includes(`induk`)||e.includes(`id`)),s=a.findIndex(e=>e.includes(`nama`)&&!e.includes(`orang`)&&!e.includes(`wali`));s===-1&&(s=a.findIndex(e=>e===`nama`||e===`name`||e.includes(`siswa`)));let c=a.findIndex(e=>e.includes(`kelas`)||e.includes(`tingkat`)||e.includes(`rombel`)),l=a.findIndex(e=>e.includes(`orang`)||e.includes(`wali`)||e.includes(`ortu`)||e.includes(`parent`)),u=a.findIndex(e=>e.includes(`hp`)||e.includes(`wa`)||e.includes(`telepon`)||e.includes(`phone`)),d=a.findIndex(e=>e.includes(`spp`)||e.includes(`nominal`)||e.includes(`biaya`)||e.includes(`tarif`));o===-1&&(o=0),s===-1&&(s=1),c===-1&&(c=2),l===-1&&(l=3),u===-1&&(u=4),d===-1&&(d=5);let f=[],p=[];for(let e=1;e<r.length;e++){let n=r[e].trim();if(!n)continue;let a=U(n,i);if(a.length===0||a.every(e=>!e))continue;let m=a[o]?.replace(/['"]/g,``).trim(),h=a[s]?.replace(/['"]/g,``).trim(),g=a[c]?.replace(/['"]/g,``).trim()||`VII-A`,_=a[l]?.replace(/['"]/g,``).trim()||`-`,v=a[u]?.replace(/['"]/g,``).trim()||``;v=v.replace(/[^\d+]/g,``),v.startsWith(`+62`)?v=`0`+v.slice(3):v.startsWith(`62`)&&(v=`0`+v.slice(2));let y=a[d]?.replace(/[^0-9]/g,``),b=y?parseInt(y,10):t;if(!m){p.push(`Baris ${e+1}: NIS tidak boleh kosong.`);continue}if(!h){p.push(`Baris ${e+1} (NIS ${m}): Nama siswa tidak boleh kosong.`);continue}f.push({nis:m,nama:h,kelas:g,namaOrangTua:_,noHp:v,nominalSpp:isNaN(b)||b<=0?t:b})}return{valid:f,errors:p,totalRows:r.length-1}}function W(e){if(!e)return``;let t=e.replace(/\D/g,``);return t.startsWith(`0`)?t=`62`+t.slice(1):t.startsWith(`8`)&&(t=`62`+t),t}function G(e,t){let n=W(e),r=encodeURIComponent(t),i=n?`https://api.whatsapp.com/send?phone=${n}&text=${r}`:`https://api.whatsapp.com/send?text=${r}`;window.open(i,`_blank`)}function K(e,t){let n=E.getSchoolInfo(),r=t?.namaOrangTua?`Bpk/Ibu ${t.namaOrangTua}`:`Wali dari ${e.nama}`,i=e.rincianItemText||`SPP Bulan ${e.bulan} ${e.tahun}`,a=``;return a=e.items&&e.items.length>0?e.items.map((e,t)=>`  ${t+1}. ${e.nama}: *${h(e.nominal)}*`).join(`
...
*${n.namaSekolah}*`}function q(e,t,n,r=``){let i=E.getSchoolInfo(),a=e.namaOrangTua?`Bpk/Ibu ${e.namaOrangTua}`:`Bpk/Ibu Wali`,o=t.length>0?t.join(`, `):`SPP Berjalan`;return`*PEMBERITAHUAN ADMINISTRASI SPP*
...
*${i.namaSekolah}*
📞 ${i.noTelepon||`-`}`}var J=[];function oe(){let e=n(`div`,{className:`page-enter`});e.innerHTML=`
  ...
  `;let t=e.querySelector(`#student-search`),r=e.querySelector(`#student-filter-kelas`),a=e.querySelector(`#btn-add-student`);return e.querySelector(`#btn-export-students`)?.addEventListener(`click`,()=>{if(J.length===0){i(`Tidak ada data siswa untuk diexport`,`warning`);return}let e=E.getSchoolInfo();V(J,e.namaSekolah),i(`Berhasil mengekspor ${J.length} siswa ke file Excel/CSV!`,`success`)}),e.querySelector(`#btn-import-students`)?.addEventListener(`click`,()=>ce(e)),t.addEventListener(`input`,()=>Y(e,t.value,r.value)),r.addEventListener(`change`,()=>Y(e,t.value,r.value)),a.addEventListener(`click`,()=>X(e)),Y(e),e}async function Y(e,t=``,n=``){let r=e.querySelector(`#students-tbody`);if(r)try{let a=await D.getStudents();if(t){let e=t.toLowerCase();a=a.filter(t=>t.nama.toLowerCase().includes(e)||t.nis.includes(e))}if(n&&(a=a.filter(e=>e.kelas===n)),J=a,a.length===0){r.innerHTML=`
        <tr>
          <td colspan="7">
    ...
    `).join(``),r.querySelectorAll(`.btn-remind-wa`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-nis`),n=a.find(e=>e.nis===t);n&&se(n)})}),r.querySelectorAll(`.btn-edit-student`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.getAttribute(`data-nis`),r=a.find(e=>e.nis===n);r&&X(e,r)})}),r.querySelectorAll(`.btn-delete-student`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=t.getAttribute(`data-nis`),r=a.find(e=>e.nis===n);r&&await c(`Yakin ingin menghapus data siswa "${r.nama}" (${r.nis})?`)&&(await D.deleteStudent(n)?(i(`Data siswa ${r.nama} berhasil dihapus`,`success`),Y(e)):i(`Gagal menghapus data siswa`,`error`))})})}catch(e){console.error(`Error loading students:`,e),r.innerHTML=`
      <tr><td colspan="7" class="text-center text-danger" style="padding: var(--space-8);">Error memuat data siswa</td></tr>
    `}}async function se(e){let t=await D.getStudentPayments(e.nis),r=new Date().getFullYear(),i=new Date().getMonth(),a=u.slice(0,i+1),c=t.filter(e=>e.tahun===r&&e.status===`lunas`).map(e=>e.bulan),l=a.filter(e=>!c.includes(e)),d=l.length*(e.nominalSpp||15e4),f=n(`div`,{innerHTML:`
    ...
    `});o(`Pengingat SPP - ${e.nama}`,f),f.querySelector(`#btn-cancel-wa-reminder`)?.addEventListener(`click`,s),f.querySelector(`#btn-send-wa-reminder`)?.addEventListener(`click`,()=>{let t=f.querySelector(`#wa-reminder-note`).value,n=q(e,l.length>0?l:[`Bulan Berjalan`],l.length>0?d:e.nominalSpp,t);G(e.noHp,n),s()})}function X(e,t){let r=!!t,a=r?`Edit Data Siswa`:`Tambah Siswa Baru`,c=n(`div`,{innerHTML:`
    ...
    `});o(`📥 Impor Data Siswa dari Excel / CSV`,a),a.querySelector(`#btn-download-template`)?.addEventListener(`click`,()=>{re(t.nominalSppDefault),i(`Template Data Siswa berhasil diunduh! Silakan buka di Microsoft Excel.`,`success`)});let c=a.querySelector(`#import-file-input`),l=a.querySelector(`#import-dropzone`),u=a.querySelector(`#import-file-name`),d=a.querySelector(`#import-preview-section`),f=a.querySelector(`#import-preview-tbody`),p=a.querySelector(`#import-status-badge`),m=a.querySelector(`#import-errors-box`),g=a.querySelector(`#btn-confirm-import`),_=a.querySelector(`#btn-cancel-import`),v=a.querySelector(`#import-update-existing`);_.addEventListener(`click`,s),l.addEventListener(`click`,()=>c.click()),l.addEventListener(`dragover`,e=>{e.preventDefault(),l.style.borderColor=`var(--color-primary)`,l.style.background=`rgba(99, 102, 241, 0.08)`}),l.addEventListener(`dragleave`,()=>{l.style.borderColor=`var(--color-border)`,l.style.background=`rgba(255, 255, 255, 0.02)`}),l.addEventListener(`drop`,e=>{e.preventDefault(),l.style.borderColor=`var(--color-border)`,l.style.background=`rgba(255, 255, 255, 0.02)`,e.dataTransfer?.files&&e.dataTransfer.files[0]&&y(e.dataTransfer.files[0])}),c.addEventListener(`change`,()=>{c.files&&c.files[0]&&y(c.files[0])});function y(e){u.style.display=`block`,u.textContent=`📄 ${e.name} (${(e.size/1024).toFixed(1)} KB)`;let n=new FileReader;n.onload=e=>{let n=e.target?.result;if(!n){i(`File tidak memiliki konten`,`warning`);return}let a=ae(n,t.nominalSppDefault);if(r=a.valid,d.style.display=`flex`,a.errors.length>0?(m.style.display=`block`,m.innerHTML=`<strong>Peringatan / Catatan (${a.errors.length}):</strong><br>`+a.errors.map(e=>`• ${e}`).join(`<br>`)):m.style.display=`none`,r.length===0){p.className=`badge badge-danger text-xs`,p.textContent=`0 Data Valid Ditemukan`,g.disabled=!0,f.innerHTML=`<tr><td colspan="6" class="text-center text-muted" style="padding: 12px;">Format tidak dikenali. Silakan periksa atau gunakan template resmi.</td></tr>`;return}p.className=`badge badge-success text-xs`,p.textContent=`✅ ${r.length} Siswa Siap Diimpor`,g.disabled=!1,g.textContent=`🚀 Impor ${r.length} Data Siswa`;let o=r.slice(0,15);f.innerHTML=o.map(e=>`
        ...
        `)},n.readAsText(e,`UTF-8`)}g.addEventListener(`click`,async()=>{if(r.length===0)return;g.disabled=!0,g.textContent=`Menyimpan ke database...`;let t=v.checked,n=await D.importStudents(r,t);i(`Berhasil mengimpor data siswa: ${n.added} siswa baru ditambahkan, ${n.updated} diperbarui!`,`success`),s(),Y(e)})}function Z(e){let t=le(e),n=document.createElement(`div`);n.innerHTML=`
    <div style="margin-bottom: var(--space-4);">
      ${t}
    </div>
  ...
  `,o(`Preview Kuitansi Pembayaran`,n),n.querySelector(`#btn-print-receipt`)?.addEventListener(`click`,()=>{ue(e)}),n.querySelector(`#btn-wa-receipt`)?.addEventListener(`click`,async()=>{try{let t=(await D.getStudents()).find(t=>t.nis===e.nis),n=K(e,t);G(t?.noHp||``,n)}catch(t){console.error(t),G(``,K(e))}})}function le(e){let t=E.getSchoolInfo(),n=e.items&&e.items.length>0,r=e.channel===`online`?`Online (Portal Siswa)`:`Kasir Administrasi Sekolah`;return`
  ...
  `}function ue(e){let t=E.getSchoolInfo(),n=window.open(``,`_blank`,`width=500,height=750`);n&&(n.document.write(`
    <!DOCTYPE html>
    <html>
    <head>
  ...
  `),n.document.close())}function de(){let e=n(`div`,{className:`page-enter`});e.innerHTML=`
    <div class="page-header">
  ...
  `,fe(e);let t=e.querySelector(`#pay-student`);t.addEventListener(`change`,()=>Q(e,t.value));let r=e.querySelector(`#grid-filter-kelas`);return r.addEventListener(`change`,()=>me(e,r.value)),e.querySelector(`#payment-form`).addEventListener(`submit`,t=>he(t,e)),e}async function fe(e){let t=e.querySelector(`#pay-student`);(await D.getStudents()).forEach(e=>{let r=n(`option`,{value:e.nis});r.textContent=`${e.nama} — ${e.kelas} (${e.nis})`,t.appendChild(r)})}async function Q(e,t){let n=e.querySelector(`#student-info-box`),r=e.querySelector(`#student-info-content`),i=e.querySelector(`#pay-nominal`),a=e.querySelector(`#payment-status-content`);if(!t){n.style.display=`none`,a.innerHTML=`<p class="text-muted" style="font-size: var(--font-size-sm);">Pilih siswa pada form untuk melihat status pembayaran.</p>`;return}let o=await D.getStudentByNis(t);o&&(n.style.display=`block`,r.innerHTML=`
    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
  ...
  `,i.value=String(o.nominalSpp),await pe(e,o))}async function pe(e,t){let n=e.querySelector(`#payment-status-content`),r=w(),i=await D.getStudentPayments(t.nis,r),a=new Set(i.filter(e=>e.status===`lunas`).map(e=>e.bulan));n.innerHTML=`
  ...
  `}async function me(e,t){let n=e.querySelector(`#payment-status-content`);if(!t){n.innerHTML=`<p class="text-muted" style="font-size: var(--font-size-sm);">Pilih kelas untuk melihat status pembayaran per kelas.</p>`;return}let r=(await D.getStudents()).filter(e=>e.kelas===t),i=await D.getPayments(),a=w();if(r.length===0){n.innerHTML=`
  ...
  `;let r=e.querySelector(`#history-search`),a=e.querySelector(`#history-channel`),o=e.querySelector(`#history-kelas`),s=e.querySelector(`#history-bulan`),c=e.querySelector(`#history-tahun`),l=()=>{let t={search:r.value,channel:a.value,kelas:o.value,bulan:s.value,tahun:Number(c.value),status:``};_e(e,t)};return r.addEventListener(`input`,l),a.addEventListener(`change`,l),o.addEventListener(`change`,l),s.addEventListener(`change`,l),c.addEventListener(`change`,l),e.querySelector(`#btn-export-history`)?.addEventListener(`click`,()=>{if($.length===0){i(`Tidak ada data transaksi untuk diexport`,`warning`);return}let e=E.getSchoolInfo();H($,e.namaSekolah),i(`Berhasil mengekspor ${$.length} transaksi ke file Excel/CSV!`,`success`)}),l(),e}async function _e(e,t){let n=e.querySelector(`#history-tbody`),r=e.querySelector(`#history-summary`);try{let i=await D.getPayments();if(t.search){let e=t.search.toLowerCase();i=i.filter(t=>t.nama.toLowerCase().includes(e)||t.nis.includes(e)||t.idTransaksi.toLowerCase().includes(e)||t.rincianItemText&&t.rincianItemText.toLowerCase().includes(e))}t.channel&&(i=i.filter(e=>(e.channel||`admin`)===t.channel)),t.kelas&&(i=i.filter(e=>e.kelas===t.kelas)),t.bulan&&(i=i.filter(e=>e.bulan===t.bulan)),t.tahun&&(i=i.filter(e=>e.tahun===t.tahun)),t.status&&(i=i.filter(e=>e.status===t.status)),i.sort((e,t)=>new Date(t.tanggalBayar).getTime()-new Date(e.tanggalBayar).getTime()),$=i;let a=i.reduce((e,t)=>e+t.nominal,0),o=i.filter(e=>e.channel===`online`).length,s=i.filter(e=>e.channel===`online`).reduce((e,t)=>e+t.nominal,0);if(r.innerHTML=`
    ...
    `,i.length===0){n.innerHTML=`
      ...
      `}).join(``),ve(e,i,t)}catch(e){console.error(`Error loading history:`,e),n.innerHTML=`
      <tr><td colspan="9" class="text-center text-danger" style="padding: var(--space-8);">Error memuat riwayat</td></tr>
    `}}function ve(e,t,n){e.querySelectorAll(`.btn-view-items`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.getAttribute(`data-id`),r=t.find(e=>e.idTransaksi===n);r&&ye(r)})}),e.querySelectorAll(`.btn-print`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.getAttribute(`data-id`),r=t.find(e=>e.idTransaksi===n);r&&Z(r)})}),e.querySelectorAll(`.btn-wa-send`).forEach(e=>{e.addEventListener(`click`,async()=>{let n=e.getAttribute(`data-id`),r=t.find(e=>e.idTransaksi===n);if(r)try{let e=(await D.getStudents()).find(e=>e.nis===r.nis),t=K(r,e);G(e?.noHp||``,t)}catch(e){console.error(e),G(``,K(r))}})}),e.querySelectorAll(`.btn-delete-payment`).forEach(r=>{r.addEventListener(`click`,async()=>{let a=r.getAttribute(`data-id`),o=t.find(e=>e.idTransaksi===a);o&&await c(`Yakin ingin menghapus pembayaran ${o.nama}?\n\nID: ${o.idTransaksi}\nNominal: ${h(o.nominal)}`)&&(await D.deletePayment(a)?(i(`Pembayaran berhasil dihapus`,`success`),_e(e,n)):i(`Gagal menghapus pembayaran`,`error`))})})}function ye(e){let t=document.createElement(`div`),n=e.channel===`online`,r=e.items&&e.items.length>0;t.innerHTML=`
    <div style="margin-bottom: var(--space-4);">
  ...
  `,o(`Rincian Pembayaran — ${e.idTransaksi}`,t),t.querySelector(`#modal-print-btn`)?.addEventListener(`click`,()=>{Z(e)})}function be(){let t=n(`div`,{className:`page-enter`}),r=E.getSchoolInfo(),a=null,o=new Map,s=new Set,c=[];t.innerHTML=`
    <!-- Hero Banner -->
  ...
  `;let l=t.querySelector(`#portal-login-card`),d=t.querySelector(`#portal-login-form`),f=t.querySelector(`#portal-input-identity`),p=t.querySelector(`#quick-student-chips`),m=t.querySelector(`#portal-session-area`),g=t.querySelector(`#session-student-name`),b=t.querySelector(`#session-student-meta`),x=t.querySelector(`#btn-switch-student`),S=t.querySelector(`#tab-btn-tagihan`),C=t.querySelector(`#tab-btn-riwayat`),T=t.querySelector(`#tab-content-tagihan`),ee=t.querySelector(`#tab-content-riwayat`),te=t.querySelector(`#count-history-badge`),A=t.querySelector(`#student-history-tbody`),j=t.querySelector(`#total-paid-summary`),M=t.querySelector(`#portal-spp-months-container`),N=t.querySelector(`#portal-other-items-container`),ne=t.querySelector(`#spp-rate-badge`),P=t.querySelector(`#check-custom-amount`),F=t.querySelector(`#custom-amount-inputs`),I=t.querySelector(`#custom-bill-nominal`),L=t.querySelector(`#custom-bill-name`),R=t.querySelector(`#summary-items-list`),z=t.querySelector(`#summary-total-amount`),B=t.querySelector(`#selected-count-badge`),V=t.querySelector(`#btn-process-online-pay`),H=t.querySelector(`#payment-gateway-modal`),re=t.querySelector(`#gateway-modal-title`),U=t.querySelector(`#gateway-modal-body`),ie=t.querySelector(`#btn-close-gateway`),ae=k.getCurrentStudent();ae?K(ae):G(),d.addEventListener(`submit`,async e=>{e.preventDefault();let t=f.value.trim();t&&await q(t)}),x.addEventListener(`click`,()=>{J()}),S.addEventListener(`click`,()=>{S.className=`btn btn-primary`,C.className=`btn btn-secondary`,T.style.display=`block`,ee.style.display=`none`}),C.addEventListener(`click`,()=>{S.className=`btn btn-secondary`,C.className=`btn btn-primary`,T.style.display=`none`,ee.style.display=`block`,ce()}),t.querySelectorAll(`input[name="portal-payment-method"]`).forEach(e=>{e.addEventListener(`change`,()=>{t.querySelectorAll(`.method-option`).forEach(e=>e.classList.remove(`active`)),e.closest(`.method-option`)?.classList.add(`active`)})}),P.addEventListener(`change`,()=>{P.checked?(F.style.display=`block`,W()):(F.style.display=`none`,o.delete(`item-custom`),X())}),I.addEventListener(`input`,()=>{P.checked&&W()}),L.addEventListener(`input`,()=>{P.checked&&W()});function W(){let e=Math.max(0,Number(I.value)||0),t=L.value.trim()||`Tagihan Bebas / Donasi`;o.set(`item-custom`,{id:`item-custom`,nama:t,kategori:`bebas`,nominal:e,keterangan:`Nominal bebas ditentukan siswa`}),X()}V.addEventListener(`click`,()=>{if(a){if(o.size===0){i(`Pilih minimal satu pos tagihan yang ingin dibayar`,`warning`);return}le(t.querySelector(`input[name="portal-payment-method"]:checked`)?.value||`qris`)}}),ie.addEventListener(`click`,()=>{H.style.display=`none`});async function G(){let e=await D.getStudents();if(e.length===0){p.innerHTML=`<span class="text-muted text-xs">Belum ada data siswa terdaftar.</span>`;return}p.innerHTML=``,e.forEach(e=>{let t=n(`button`,{className:`btn btn-ghost btn-sm`,style:`border: 1px solid var(--color-border); border-radius: var(--radius-full); padding: 4px 12px; font-size: 11px;`,innerHTML:`👨‍🎓 ${e.nama} (${e.nis})`});t.addEventListener(`click`,()=>{q(e.nis)}),p.appendChild(t)})}async function K(e){a=e,l.style.display=`none`,m.style.display=`block`,g.textContent=e.nama,b.innerHTML=`<strong>NIS:</strong> ${e.nis} • <strong>Kelas:</strong> ${e.kelas} • <strong>Wali:</strong> ${e.namaOrangTua||`-`} • <strong>HP:</strong> ${e.noHp||`-`}`,ne.textContent=`${h(e.nominalSpp)} / bln`,await oe()}async function q(e){let t=await k.loginAsStudent(e);if(!t.success||!t.student){i(t.error||`Siswa tidak ditemukan`,`error`);return}await K(t.student),i(`Selamat datang, ${t.student.nama}!`,`success`)}function J(){a=null,k.getRole()===`admin`?(m.style.display=`none`,l.style.display=`block`,f.value=``,o.clear(),X(),i(`Selesai pratinjau akun siswa`,`info`)):(k.logout(),i(`Anda telah keluar dari akun siswa`,`info`),e.navigate(`/login`))}async function oe(){a&&(c=await D.getStudentPayments(a.nis),s.clear(),c.forEach(e=>{e.status===`lunas`&&(s.add(e.bulan),e.items&&e.items.forEach(e=>{e.kategori===`spp`&&e.bulan&&s.add(e.bulan)}))}),te.textContent=String(c.length),o.clear(),P.checked=!1,F.style.display=`none`,Y(),se(),X())}function Y(){M.innerHTML=``;let e=a?.nominalSpp||E.getSchoolInfo().nominalSppDefault;u.forEach(t=>{let r=s.has(t),i=`spp-${t}-${w()}`,a=o.has(i),c=n(`label`,{className:`portal-month-card ${r?`paid`:``} ${a?`selected`:``}`});if(c.innerHTML=`
    ...
    `).join(``)}function ce(){if(!a)return;let e=c.reduce((e,t)=>e+t.nominal,0);if(j.textContent=`Total Lunas: ${h(e)}`,c.length===0){A.innerHTML=`
        <tr>
          <td colspan="7">
            <div class="empty-state" style="padding: var(--space-8) var(--space-4);">
              <div class="empty-state-icon">📭</div>
    ...
    `,re.textContent=`Bukti Pembayaran — ${n.namaSekolah}`,U.innerHTML=``,U.appendChild(r),H.style.display=`flex`,r.querySelector(`#btn-print-from-success`)?.addEventListener(`click`,()=>{Z(e)}),r.querySelector(`#btn-done-success`)?.addEventListener(`click`,()=>{H.style.display=`none`,C.click()})}return t}function xe(){let e=n(`div`,{className:`page-enter`});e.innerHTML=`
    <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: var(--space-4);">
      <div>
  ...
  `;let a=t.querySelector(`#tab-login-siswa`),o=t.querySelector(`#tab-login-admin`),s=t.querySelector(`#box-login-siswa`),c=t.querySelector(`#box-login-admin`),l=t.querySelector(`#form-login-siswa`),u=t.querySelector(`#form-login-admin`),d=t.querySelector(`#input-siswa-identity`),f=t.querySelector(`#input-admin-user`),p=t.querySelector(`#input-admin-pass`),m=t.querySelector(`#login-quick-chips`);a.addEventListener(`click`,()=>{a.classList.add(`active`),o.classList.remove(`active`),s.style.display=`block`,c.style.display=`none`,d.focus()}),o.addEventListener(`click`,()=>{o.classList.add(`active`),a.classList.remove(`active`),c.style.display=`block`,s.style.display=`none`,p.focus()}),h();async function h(){let e=await D.getStudents();if(e.length===0){m.innerHTML=`<span class="text-xs text-muted">Belum ada data siswa terdaftar.</span>`;return}m.innerHTML=``,e.forEach(e=>{let t=n(`button`,{className:`btn btn-ghost btn-sm`,style:`border: 1px solid var(--color-border); border-radius: var(--radius-full); padding: 4px 12px; font-size: 11px;`,innerHTML:`👨‍🎓 ${e.nama} (${e.nis})`});t.addEventListener(`click`,()=>{g(e.nis)}),m.appendChild(t)})}l.addEventListener(`submit`,async e=>{e.preventDefault();let t=d.value.trim();t&&await g(t)});async function g(t){let n=await k.loginAsStudent(t);if(!n.success){i(n.error||`Gagal masuk sebagai siswa`,`error`);return}i(`Selamat datang, ${n.student?.nama}!`,`success`),e.navigate(`/portal-siswa`)}return u.addEventListener(`submit`,t=>{t.preventDefault();let n=f.value.trim(),r=p.value,a=k.loginAsAdmin(n,r);if(!a.success){i(a.error||`Login admin gagal`,`error`);return}i(`Login berhasil sebagai Administrator!`,`success`),e.navigate(`/`)}),t}function we(){e.registerAll([{path:`/login`,title:`Masuk Akun`,icon:`🔑`,render:Ce},{path:`/`,title:`Dashboard`,icon:`🏠`,render:F},{path:`/portal-siswa`,title:`Portal Siswa`,icon:`💳`,render:be},{path:`/pembayaran`,title:`Kasir Pembayaran`,icon:`💰`,render:de},{path:`/riwayat`,title:`Riwayat Pembayaran`,icon:`📋`,render:ge},{path:`/siswa`,title:`Data Siswa`,icon:`👨‍🎓`,render:oe},{path:`/pos-pembayaran`,title:`Pos Pembayaran`,icon:`🏷️`,render:xe},{path:`/identitas-sekolah`,title:`Identitas Sekolah`,icon:`⚙️`,render:Se}]);let n=t(`#app`);if(!n)return;n.innerHTML=``,n.classList.add(`app-shell`),n.appendChild(M()),n.appendChild(N()),n.appendChild(te());let r=document.createElement(`main`);r.className=`main-content`,r.id=`main-content`,n.appendChild(r),e.onRouteChange(t=>{let a=k.getRole();if(!a){if(t!==`/login`){e.navigate(`/login`);return}}else if(a===`siswa`){if(t!==`/portal-siswa`){i(`Akses dibatasi. Siswa hanya dapat mengakses Portal Pembayaran mandiri.`,`warning`),e.navigate(`/portal-siswa`);return}}else if(a===`admin`&&t===`/login`){e.navigate(`/`);return}let o=t===`/login`;n.classList.toggle(`login-mode`,o),window.dispatchEvent(new CustomEvent(`app:auth-changed`));let s=e.getCurrentRoute();if(s){j(t);let e=E.getSchoolInfo();if(document.title=`${s.title} — ${e.namaSekolah}`,r.innerHTML=``,!o&&a===`admin`&&t===`/`&&!D.useApi){let e=document.createElement(`div`);e.className=`config-banner animate-fade-in-down`,e.innerHTML=`
        ...
        `,r.appendChild(e)}r.appendChild(s.render())}}),e.start(),O.requestPermission(),setTimeout(()=>{O.checkAndNotify()},3e3)}document.addEventListener(`DOMContentLoaded`,we);