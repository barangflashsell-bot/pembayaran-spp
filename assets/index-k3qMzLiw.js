(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=new class{routes=new Map;listeners=[];currentPath=``;register(e){this.routes.set(e.path,e)}registerAll(e){e.forEach(e=>this.register(e))}navigate(e){window.location.hash=`#${e}`}getCurrentPath(){return window.location.hash.slice(1)||`/`}getCurrentRoute(){return this.routes.get(this.getCurrentPath())}getRoutes(){return Array.from(this.routes.values())}onRouteChange(e){this.listeners.push(e)}start(){let e=()=>{let e=this.getCurrentPath();e!==this.currentPath&&(this.currentPath=e,this.listeners.forEach(t=>t(e)))};window.addEventListener(`hashchange`,e),window.location.hash?e():this.navigate(`/`)}};function t(e,t=document){return t.querySelector(e)}function n(e,t,...n){let r=document.createElement(e);if(t)for(let[e,n]of Object.entries(t))e===`className`?r.className=n:e===`innerHTML`?r.innerHTML=n:e===`textContent`?r.textContent=n:r.setAttribute(e,n);for(let e of n)typeof e==`string`?r.appendChild(document.createTextNode(e)):r.appendChild(e);return r}var r={success:`✅`,error:`❌`,warning:`⚠️`,info:`ℹ️`};function i(e,i=`info`,o=4e3){let s=t(`.toast-container`);s||(s=n(`div`,{className:`toast-container`}),document.body.appendChild(s));let c=n(`div`,{className:`toast toast-${i} toast-enter`,innerHTML:`
  ...
  `,n.querySelector(`#btn-mark-all-read`)?.addEventListener(`click`,()=>{O.markAllAsRead(),e(),A(e)}),n.querySelector(`#btn-clear-all-notifs`)?.addEventListener(`click`,()=>{O.clearNotifications(),e(),A(e)}),o(`🔔 Pusat Notifikasi Admin`,n)}function ne(e){document.querySelectorAll(`.nav-link`).forEach(t=>{let n=t.getAttribute(`data-path`);t.classList.toggle(`active`,n===e)})}function j(){let e=n(`button`,{className:`mobile-menu-btn`,id:`mobile-menu-btn`,innerHTML:`☰`});return e.addEventListener(`click`,re),e}function M(){let e=n(`div`,{className:`sidebar-overlay`,id:`sidebar-overlay`});return e.addEventListener(`click`,N),e}function re(){let e=t(`#sidebar`),n=t(`#sidebar-overlay`);e?.classList.toggle(`open`),n?.classList.toggle(`visible`)}function N(){let e=t(`#sidebar`),n=t(`#sidebar-overlay`);e?.classList.remove(`open`),n?.classList.remove(`visible`)}function P(){let e=n(`div`,{className:`page-enter`});return e.innerHTML=`
  ...
  `}async function I(e){let t=u[C()],n=w();try{let[r,i,a]=await Promise.all([D.getDashboardStats(t,n),D.getUnpaidStudents(t,n),D.getPayments()]),o=a.filter(e=>e.channel===`online`),s=o.reduce((e,t)=>e+t.nominal,0),c=e.querySelector(`#stats-container`);c&&(c.innerHTML=``,c.classList.add(`stagger-children`),c.appendChild(L(`👨‍🎓`,`Total Siswa`,r.totalSiswa.toString(),`Terdaftar aktif`,`primary`)),c.appendChild(L(`✅`,`Sudah Bayar SPP`,r.sudahBayar.toString(),`Bulan ${t}`,`success`)),c.appendChild(L(`⚠️`,`Belum Bayar SPP`,r.belumBayar.toString(),`Bulan ${t}`,`warning`)),c.appendChild(L(`💳`,`Setoran Online`,h(s),`${o.length} transaksi online`,`primary`)),c.appendChild(L(`💰`,`Total Pemasukan`,h(r.totalPemasukan),`Semua pos tagihan`,`info`)));let l=e.querySelector(`#unpaid-list`);l&&(l.classList.remove(`animate-pulse`),i.length===0?l.innerHTML=`
    ...
    `})}function R(e,t){let n=new Blob([`﻿`+t],{type:`text/csv;charset=utf-8;`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.setAttribute(`href`,r),i.setAttribute(`download`,e.endsWith(`.csv`)?e:`${e}.csv`),document.body.appendChild(i),i.click(),document.body.removeChild(i),URL.revokeObjectURL(r)}function z(e){return e==null?`""`:`"${String(e).replace(/"/g,`""`)}"`}function B(e,t=`SMP Nusantara Unggul`){let n=[`No`,`NIS`,`Nama Siswa`,`Kelas`,`Nama Orang Tua / Wali`,`No. WhatsApp / HP`,`Nominal SPP (Rp)`],r=e.map((e,t)=>[t+1,e.nis,e.nama,e.kelas,e.namaOrangTua||`-`,e.noHp||`-`,e.nominalSpp]),i=[[`Data Siswa - ${t}`],[`Diunduh pada: ${new Date().toLocaleString(`id-ID`)}`],[],n.map(z).join(`,`),...r.map(e=>e.map(z).join(`,`))].map(e=>Array.isArray(e)?e.join(`,`):e).join(`\r
`);R(`Data_Siswa_${new Date().toISOString().slice(0,10)}.csv`,i)}function V(e,t=`SMP Nusantara Unggul`){let n=[`No`,`ID Transaksi`,`Tanggal Bayar`,`NIS`,`Nama Siswa`,`Kelas`,`Rincian Pos / Item Dibayar`,`Nominal (Rp)`,`Channel`,`Metode Bayar`,`Status`,`Catatan / Keterangan`],r=e.map((e,t)=>[t+1,e.idTransaksi,g(e.tanggalBayar),e.nis,e.nama,e.kelas,e.rincianItemText||`SPP ${e.bulan} ${e.tahun}`,e.nominal,e.channel===`online`?`Online (Siswa)`:`Kasir Admin`,e.metodeBayar.toUpperCase(),e.status.toUpperCase(),e.keterangan||`-`]),i=e.reduce((e,t)=>e+(t.nominal||0),0),a=[[`Laporan Rekap Transaksi Pembayaran - ${t}`],[`Tanggal Cetak: ${new Date().toLocaleString(`id-ID`)}`],[`Total Transaksi: ${e.length} | Total Pemasukan: ${h(i)}`],[],n.map(z).join(`,`),...r.map(e=>e.map(z).join(`,`)),[],[``,``,``,``,``,`TOTAL DITERIMA`,``,i,``,``,``,``].map(z).join(`,`)].map(e=>Array.isArray(e)?e.join(`,`):e).join(`\r
`);R(`Laporan_Pembayaran_${new Date().toISOString().slice(0,10)}.csv`,a)}function ie(e){if(!e)return``;let t=e.replace(/\D/g,``);return t.startsWith(`0`)?t=`62`+t.slice(1):t.startsWith(`8`)&&(t=`62`+t),t}function H(e,t){let n=ie(e),r=encodeURIComponent(t),i=n?`https://api.whatsapp.com/send?phone=${n}&text=${r}`:`https://api.whatsapp.com/send?text=${r}`;window.open(i,`_blank`)}function U(e,t){let n=E.getSchoolInfo(),r=t?.namaOrangTua?`Bpk/Ibu ${t.namaOrangTua}`:`Wali dari ${e.nama}`,i=e.rincianItemText||`SPP Bulan ${e.bulan} ${e.tahun}`,a=``;return a=e.items&&e.items.length>0?e.items.map((e,t)=>`  ${t+1}. ${e.nama}: *${h(e.nominal)}*`).join(`
...
*${n.namaSekolah}*`}function W(e,t,n,r=``){let i=E.getSchoolInfo(),a=e.namaOrangTua?`Bpk/Ibu ${e.namaOrangTua}`:`Bpk/Ibu Wali`,o=t.length>0?t.join(`, `):`SPP Berjalan`;return`*PEMBERITAHUAN ADMINISTRASI SPP*
...
*${i.namaSekolah}*
📞 ${i.noTelepon||`-`}`}var G=[];function ae(){let e=n(`div`,{className:`page-enter`});e.innerHTML=`
  ...
  `;let t=e.querySelector(`#student-search`),r=e.querySelector(`#student-filter-kelas`),a=e.querySelector(`#btn-add-student`);return e.querySelector(`#btn-export-students`)?.addEventListener(`click`,()=>{if(G.length===0){i(`Tidak ada data siswa untuk diexport`,`warning`);return}let e=E.getSchoolInfo();B(G,e.namaSekolah),i(`Berhasil mengekspor ${G.length} siswa ke file Excel/CSV!`,`success`)}),t.addEventListener(`input`,()=>K(e,t.value,r.value)),r.addEventListener(`change`,()=>K(e,t.value,r.value)),a.addEventListener(`click`,()=>J(e)),K(e),e}async function K(e,t=``,n=``){let r=e.querySelector(`#students-tbody`);if(r)try{let a=await D.getStudents();if(t){let e=t.toLowerCase();a=a.filter(t=>t.nama.toLowerCase().includes(e)||t.nis.includes(e))}if(n&&(a=a.filter(e=>e.kelas===n)),G=a,a.length===0){r.innerHTML=`
        <tr>
          <td colspan="7">
    ...
    `).join(``),r.querySelectorAll(`.btn-remind-wa`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-nis`),n=a.find(e=>e.nis===t);n&&q(n)})}),r.querySelectorAll(`.btn-edit-student`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.getAttribute(`data-nis`),r=a.find(e=>e.nis===n);r&&J(e,r)})}),r.querySelectorAll(`.btn-delete-student`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=t.getAttribute(`data-nis`),r=a.find(e=>e.nis===n);r&&await c(`Yakin ingin menghapus data siswa "${r.nama}" (${r.nis})?`)&&(await D.deleteStudent(n)?(i(`Data siswa ${r.nama} berhasil dihapus`,`success`),K(e)):i(`Gagal menghapus data siswa`,`error`))})})}catch(e){console.error(`Error loading students:`,e),r.innerHTML=`
      <tr><td colspan="7" class="text-center text-danger" style="padding: var(--space-8);">Error memuat data siswa</td></tr>
    `}}async function q(e){let t=await D.getStudentPayments(e.nis),r=new Date().getFullYear(),i=new Date().getMonth(),a=u.slice(0,i+1),c=t.filter(e=>e.tahun===r&&e.status===`lunas`).map(e=>e.bulan),l=a.filter(e=>!c.includes(e)),d=l.length*(e.nominalSpp||15e4),f=n(`div`,{innerHTML:`
    ...
    `});o(`Pengingat SPP - ${e.nama}`,f),f.querySelector(`#btn-cancel-wa-reminder`)?.addEventListener(`click`,s),f.querySelector(`#btn-send-wa-reminder`)?.addEventListener(`click`,()=>{let t=f.querySelector(`#wa-reminder-note`).value,n=W(e,l.length>0?l:[`Bulan Berjalan`],l.length>0?d:e.nominalSpp,t);H(e.noHp,n),s()})}function J(e,t){let r=!!t,a=r?`Edit Data Siswa`:`Tambah Siswa Baru`,c=n(`div`,{innerHTML:`
  ...
  `,o(`Preview Kuitansi Pembayaran`,n),n.querySelector(`#btn-print-receipt`)?.addEventListener(`click`,()=>{oe(e)}),n.querySelector(`#btn-wa-receipt`)?.addEventListener(`click`,async()=>{try{let t=(await D.getStudents()).find(t=>t.nis===e.nis),n=U(e,t);H(t?.noHp||``,n)}catch(t){console.error(t),H(``,U(e))}})}function X(e){let t=E.getSchoolInfo(),n=e.items&&e.items.length>0,r=e.channel===`online`?`Online (Portal Siswa)`:`Kasir Administrasi Sekolah`;return`
  ...
  `}function oe(e){let t=E.getSchoolInfo(),n=window.open(``,`_blank`,`width=500,height=750`);n&&(n.document.write(`
    <!DOCTYPE html>
    <html>
    <head>
  ...
  `),n.document.close())}function se(){let e=n(`div`,{className:`page-enter`});e.innerHTML=`
    <div class="page-header">
  ...
  `,Z(e);let t=e.querySelector(`#pay-student`);t.addEventListener(`change`,()=>ce(e,t.value));let r=e.querySelector(`#grid-filter-kelas`);return r.addEventListener(`change`,()=>ue(e,r.value)),e.querySelector(`#payment-form`).addEventListener(`submit`,t=>de(t,e)),e}async function Z(e){let t=e.querySelector(`#pay-student`);(await D.getStudents()).forEach(e=>{let r=n(`option`,{value:e.nis});r.textContent=`${e.nama} — ${e.kelas} (${e.nis})`,t.appendChild(r)})}async function ce(e,t){let n=e.querySelector(`#student-info-box`),r=e.querySelector(`#student-info-content`),i=e.querySelector(`#pay-nominal`),a=e.querySelector(`#payment-status-content`);if(!t){n.style.display=`none`,a.innerHTML=`<p class="text-muted" style="font-size: var(--font-size-sm);">Pilih siswa pada form untuk melihat status pembayaran.</p>`;return}let o=await D.getStudentByNis(t);o&&(n.style.display=`block`,r.innerHTML=`
    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
  ...
  `,i.value=String(o.nominalSpp),await le(e,o))}async function le(e,t){let n=e.querySelector(`#payment-status-content`),r=w(),i=await D.getStudentPayments(t.nis,r),a=new Set(i.filter(e=>e.status===`lunas`).map(e=>e.bulan));n.innerHTML=`
  ...
  `}async function ue(e,t){let n=e.querySelector(`#payment-status-content`);if(!t){n.innerHTML=`<p class="text-muted" style="font-size: var(--font-size-sm);">Pilih kelas untuk melihat status pembayaran per kelas.</p>`;return}let r=(await D.getStudents()).filter(e=>e.kelas===t),i=await D.getPayments(),a=w();if(r.length===0){n.innerHTML=`
  ...
  `;let r=e.querySelector(`#history-search`),a=e.querySelector(`#history-channel`),o=e.querySelector(`#history-kelas`),s=e.querySelector(`#history-bulan`),c=e.querySelector(`#history-tahun`),l=()=>{let t={search:r.value,channel:a.value,kelas:o.value,bulan:s.value,tahun:Number(c.value),status:``};fe(e,t)};return r.addEventListener(`input`,l),a.addEventListener(`change`,l),o.addEventListener(`change`,l),s.addEventListener(`change`,l),c.addEventListener(`change`,l),e.querySelector(`#btn-export-history`)?.addEventListener(`click`,()=>{if(Q.length===0){i(`Tidak ada data transaksi untuk diexport`,`warning`);return}let e=E.getSchoolInfo();V(Q,e.namaSekolah),i(`Berhasil mengekspor ${Q.length} transaksi ke file Excel/CSV!`,`success`)}),l(),e}async function fe(e,t){let n=e.querySelector(`#history-tbody`),r=e.querySelector(`#history-summary`);try{let i=await D.getPayments();if(t.search){let e=t.search.toLowerCase();i=i.filter(t=>t.nama.toLowerCase().includes(e)||t.nis.includes(e)||t.idTransaksi.toLowerCase().includes(e)||t.rincianItemText&&t.rincianItemText.toLowerCase().includes(e))}t.channel&&(i=i.filter(e=>(e.channel||`admin`)===t.channel)),t.kelas&&(i=i.filter(e=>e.kelas===t.kelas)),t.bulan&&(i=i.filter(e=>e.bulan===t.bulan)),t.tahun&&(i=i.filter(e=>e.tahun===t.tahun)),t.status&&(i=i.filter(e=>e.status===t.status)),i.sort((e,t)=>new Date(t.tanggalBayar).getTime()-new Date(e.tanggalBayar).getTime()),Q=i;let a=i.reduce((e,t)=>e+t.nominal,0),o=i.filter(e=>e.channel===`online`).length,s=i.filter(e=>e.channel===`online`).reduce((e,t)=>e+t.nominal,0);if(r.innerHTML=`
    ...
    `,i.length===0){n.innerHTML=`
      ...
      `}).join(``),pe(e,i,t)}catch(e){console.error(`Error loading history:`,e),n.innerHTML=`
      <tr><td colspan="9" class="text-center text-danger" style="padding: var(--space-8);">Error memuat riwayat</td></tr>
    `}}function pe(e,t,n){e.querySelectorAll(`.btn-view-items`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.getAttribute(`data-id`),r=t.find(e=>e.idTransaksi===n);r&&me(r)})}),e.querySelectorAll(`.btn-print`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.getAttribute(`data-id`),r=t.find(e=>e.idTransaksi===n);r&&Y(r)})}),e.querySelectorAll(`.btn-wa-send`).forEach(e=>{e.addEventListener(`click`,async()=>{let n=e.getAttribute(`data-id`),r=t.find(e=>e.idTransaksi===n);if(r)try{let e=(await D.getStudents()).find(e=>e.nis===r.nis),t=U(r,e);H(e?.noHp||``,t)}catch(e){console.error(e),H(``,U(r))}})}),e.querySelectorAll(`.btn-delete-payment`).forEach(r=>{r.addEventListener(`click`,async()=>{let a=r.getAttribute(`data-id`),o=t.find(e=>e.idTransaksi===a);o&&await c(`Yakin ingin menghapus pembayaran ${o.nama}?\n\nID: ${o.idTransaksi}\nNominal: ${h(o.nominal)}`)&&(await D.deletePayment(a)?(i(`Pembayaran berhasil dihapus`,`success`),fe(e,n)):i(`Gagal menghapus pembayaran`,`error`))})})}function me(e){let t=document.createElement(`div`),n=e.channel===`online`,r=e.items&&e.items.length>0;t.innerHTML=`
    <div style="margin-bottom: var(--space-4);">
  ...
  `,o(`Rincian Pembayaran — ${e.idTransaksi}`,t),t.querySelector(`#modal-print-btn`)?.addEventListener(`click`,()=>{Y(e)})}function he(){let t=n(`div`,{className:`page-enter`}),r=E.getSchoolInfo(),a=null,o=new Map,s=new Set,c=[];t.innerHTML=`
    <!-- Hero Banner -->
  ...
  `;let l=t.querySelector(`#portal-login-card`),d=t.querySelector(`#portal-login-form`),f=t.querySelector(`#portal-input-identity`),p=t.querySelector(`#quick-student-chips`),m=t.querySelector(`#portal-session-area`),g=t.querySelector(`#session-student-name`),b=t.querySelector(`#session-student-meta`),x=t.querySelector(`#btn-switch-student`),S=t.querySelector(`#tab-btn-tagihan`),C=t.querySelector(`#tab-btn-riwayat`),T=t.querySelector(`#tab-content-tagihan`),ee=t.querySelector(`#tab-content-riwayat`),te=t.querySelector(`#count-history-badge`),A=t.querySelector(`#student-history-tbody`),ne=t.querySelector(`#total-paid-summary`),j=t.querySelector(`#portal-spp-months-container`),M=t.querySelector(`#portal-other-items-container`),re=t.querySelector(`#spp-rate-badge`),N=t.querySelector(`#check-custom-amount`),P=t.querySelector(`#custom-amount-inputs`),F=t.querySelector(`#custom-bill-nominal`),I=t.querySelector(`#custom-bill-name`),L=t.querySelector(`#summary-items-list`),R=t.querySelector(`#summary-total-amount`),z=t.querySelector(`#selected-count-badge`),B=t.querySelector(`#btn-process-online-pay`),V=t.querySelector(`#payment-gateway-modal`),ie=t.querySelector(`#gateway-modal-title`),H=t.querySelector(`#gateway-modal-body`),U=t.querySelector(`#btn-close-gateway`),W=k.getCurrentStudent();W?K(W):ae(),d.addEventListener(`submit`,async e=>{e.preventDefault();let t=f.value.trim();t&&await q(t)}),x.addEventListener(`click`,()=>{J()}),S.addEventListener(`click`,()=>{S.className=`btn btn-primary`,C.className=`btn btn-secondary`,T.style.display=`block`,ee.style.display=`none`}),C.addEventListener(`click`,()=>{S.className=`btn btn-secondary`,C.className=`btn btn-primary`,T.style.display=`none`,ee.style.display=`block`,ce()}),t.querySelectorAll(`input[name="portal-payment-method"]`).forEach(e=>{e.addEventListener(`change`,()=>{t.querySelectorAll(`.method-option`).forEach(e=>e.classList.remove(`active`)),e.closest(`.method-option`)?.classList.add(`active`)})}),N.addEventListener(`change`,()=>{N.checked?(P.style.display=`block`,G()):(P.style.display=`none`,o.delete(`item-custom`),Z())}),F.addEventListener(`input`,()=>{N.checked&&G()}),I.addEventListener(`input`,()=>{N.checked&&G()});function G(){let e=Math.max(0,Number(F.value)||0),t=I.value.trim()||`Tagihan Bebas / Donasi`;o.set(`item-custom`,{id:`item-custom`,nama:t,kategori:`bebas`,nominal:e,keterangan:`Nominal bebas ditentukan siswa`}),Z()}B.addEventListener(`click`,()=>{if(a){if(o.size===0){i(`Pilih minimal satu pos tagihan yang ingin dibayar`,`warning`);return}le(t.querySelector(`input[name="portal-payment-method"]:checked`)?.value||`qris`)}}),U.addEventListener(`click`,()=>{V.style.display=`none`});async function ae(){let e=await D.getStudents();if(e.length===0){p.innerHTML=`<span class="text-muted text-xs">Belum ada data siswa terdaftar.</span>`;return}p.innerHTML=``,e.forEach(e=>{let t=n(`button`,{className:`btn btn-ghost btn-sm`,style:`border: 1px solid var(--color-border); border-radius: var(--radius-full); padding: 4px 12px; font-size: 11px;`,innerHTML:`👨‍🎓 ${e.nama} (${e.nis})`});t.addEventListener(`click`,()=>{q(e.nis)}),p.appendChild(t)})}async function K(e){a=e,l.style.display=`none`,m.style.display=`block`,g.textContent=e.nama,b.innerHTML=`<strong>NIS:</strong> ${e.nis} • <strong>Kelas:</strong> ${e.kelas} • <strong>Wali:</strong> ${e.namaOrangTua||`-`} • <strong>HP:</strong> ${e.noHp||`-`}`,re.textContent=`${h(e.nominalSpp)} / bln`,await X()}async function q(e){let t=await k.loginAsStudent(e);if(!t.success||!t.student){i(t.error||`Siswa tidak ditemukan`,`error`);return}await K(t.student),i(`Selamat datang, ${t.student.nama}!`,`success`)}function J(){a=null,k.getRole()===`admin`?(m.style.display=`none`,l.style.display=`block`,f.value=``,o.clear(),Z(),i(`Selesai pratinjau akun siswa`,`info`)):(k.logout(),i(`Anda telah keluar dari akun siswa`,`info`),e.navigate(`/login`))}async function X(){a&&(c=await D.getStudentPayments(a.nis),s.clear(),c.forEach(e=>{e.status===`lunas`&&(s.add(e.bulan),e.items&&e.items.forEach(e=>{e.kategori===`spp`&&e.bulan&&s.add(e.bulan)}))}),te.textContent=String(c.length),o.clear(),N.checked=!1,P.style.display=`none`,oe(),se(),Z())}function oe(){j.innerHTML=``;let e=a?.nominalSpp||E.getSchoolInfo().nominalSppDefault;u.forEach(t=>{let r=s.has(t),i=`spp-${t}-${w()}`,a=o.has(i),c=n(`label`,{className:`portal-month-card ${r?`paid`:``} ${a?`selected`:``}`});if(c.innerHTML=`
      ...
      `;let a=i.querySelector(`.portal-item-checkbox`);a.addEventListener(`change`,()=>{a.checked?(o.set(t,{id:t,nama:e.nama,kategori:e.kategori,nominal:e.nominalDefault}),i.classList.add(`selected`)):(o.delete(t),i.classList.remove(`selected`)),Z()}),M.appendChild(i)})}function Z(){let e=Array.from(o.values()),t=e.length,n=e.reduce((e,t)=>e+t.nominal,0);if(z.textContent=`${t} Item`,R.textContent=h(n),t===0){L.innerHTML=`
        <div class="text-center text-muted" style="padding: var(--space-8);">
          Belum ada tagihan yang dipilih.<br>
          <small>Silakan centang item di sebelah kiri.</small>
        </div>
      `,B.disabled=!0,B.innerHTML=`🔒 Pilih Tagihan Terlebih Dahulu`;return}B.disabled=!1,B.innerHTML=`🔒 Bayar Sekarang • ${h(n)}`,L.innerHTML=e.map(e=>`
    ...
    `).join(``)}function ce(){if(!a)return;let e=c.reduce((e,t)=>e+t.nominal,0);if(ne.textContent=`Total Lunas: ${h(e)}`,c.length===0){A.innerHTML=`
        <tr>
          <td colspan="7">
            <div class="empty-state" style="padding: var(--space-8) var(--space-4);">
              <div class="empty-state-icon">📭</div>
      ...
      `;return}let t=[...c].sort((e,t)=>new Date(t.tanggalBayar).getTime()-new Date(e.tanggalBayar).getTime());A.innerHTML=t.map(e=>{let t=e.items?e.items.length:1,n=e.rincianItemText||`SPP ${e.bulan} ${e.tahun}`;return`
        <tr>
          <td><code style="font-size: var(--font-size-xs); background: var(--color-bg-glass); padding: 2px 6px; border-radius: var(--radius-sm);">${e.idTransaksi}</code></td>
          <td>${_(e.tanggalBayar)}</td>
          <td>
      ...
      `}).join(``),A.querySelectorAll(`.btn-print-receipt-student`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=c.find(e=>e.idTransaksi===t);n&&Y(n)})})}function le(e){if(!a)return;let t=Array.from(o.values()),n=t.reduce((e,t)=>e+t.nominal,0),r=y();ie.textContent=`Pembayaran Online — ${E.getSchoolInfo().namaSekolah}`,V.style.display=`flex`,e===`qris`?ue(r,n,t):e.startsWith(`va_`)?de(e,r,n,t):Q(e,r,n,t)}function ue(e,t,n){let r=E.getSchoolInfo();H.innerHTML=`
    ...
    `,H.querySelector(`#btn-simulate-success`)?.addEventListener(`click`,()=>{$(`qris`,e,t,n)})}function de(e,t,n,r){let o=E.getSchoolInfo(),s=`${e===`va_bca`?`8808`:e===`va_bri`?`1288`:`8901`}${a?.nis||`2026001`}`;H.innerHTML=`
    ...
    `,H.querySelector(`#btn-copy-va`)?.addEventListener(`click`,()=>{navigator.clipboard.writeText(s),i(`Nomor VA disalin!`,`info`)}),H.querySelector(`#btn-simulate-success`)?.addEventListener(`click`,()=>{$(e,t,n,r)})}function Q(e,t,n,r){let i=e===`dana`?`DANA`:e===`gopay`?`GoPay`:`OVO`;H.innerHTML=`
    ...
    `,ie.textContent=`Bukti Pembayaran — ${n.namaSekolah}`,H.innerHTML=``,H.appendChild(r),V.style.display=`flex`,r.querySelector(`#btn-print-from-success`)?.addEventListener(`click`,()=>{Y(e)}),r.querySelector(`#btn-done-success`)?.addEventListener(`click`,()=>{V.style.display=`none`,C.click()})}return t}function ge(){let e=n(`div`,{className:`page-enter`});e.innerHTML=`
    <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: var(--space-4);">
      <div>
  ...
  `;let a=t.querySelector(`#tab-login-siswa`),o=t.querySelector(`#tab-login-admin`),s=t.querySelector(`#box-login-siswa`),c=t.querySelector(`#box-login-admin`),l=t.querySelector(`#form-login-siswa`),u=t.querySelector(`#form-login-admin`),d=t.querySelector(`#input-siswa-identity`),f=t.querySelector(`#input-admin-user`),p=t.querySelector(`#input-admin-pass`),m=t.querySelector(`#login-quick-chips`);a.addEventListener(`click`,()=>{a.classList.add(`active`),o.classList.remove(`active`),s.style.display=`block`,c.style.display=`none`,d.focus()}),o.addEventListener(`click`,()=>{o.classList.add(`active`),a.classList.remove(`active`),c.style.display=`block`,s.style.display=`none`,p.focus()}),h();async function h(){let e=await D.getStudents();if(e.length===0){m.innerHTML=`<span class="text-xs text-muted">Belum ada data siswa terdaftar.</span>`;return}m.innerHTML=``,e.forEach(e=>{let t=n(`button`,{className:`btn btn-ghost btn-sm`,style:`border: 1px solid var(--color-border); border-radius: var(--radius-full); padding: 4px 12px; font-size: 11px;`,innerHTML:`👨‍🎓 ${e.nama} (${e.nis})`});t.addEventListener(`click`,()=>{g(e.nis)}),m.appendChild(t)})}l.addEventListener(`submit`,async e=>{e.preventDefault();let t=d.value.trim();t&&await g(t)});async function g(t){let n=await k.loginAsStudent(t);if(!n.success){i(n.error||`Gagal masuk sebagai siswa`,`error`);return}i(`Selamat datang, ${n.student?.nama}!`,`success`),e.navigate(`/portal-siswa`)}return u.addEventListener(`submit`,t=>{t.preventDefault();let n=f.value.trim(),r=p.value,a=k.loginAsAdmin(n,r);if(!a.success){i(a.error||`Login admin gagal`,`error`);return}i(`Login berhasil sebagai Administrator!`,`success`),e.navigate(`/`)}),t}function ye(){e.registerAll([{path:`/login`,title:`Masuk Akun`,icon:`🔑`,render:ve},{path:`/`,title:`Dashboard`,icon:`🏠`,render:P},{path:`/portal-siswa`,title:`Portal Siswa`,icon:`💳`,render:he},{path:`/pembayaran`,title:`Kasir Pembayaran`,icon:`💰`,render:se},{path:`/riwayat`,title:`Riwayat Pembayaran`,icon:`📋`,render:$},{path:`/siswa`,title:`Data Siswa`,icon:`👨‍🎓`,render:ae},{path:`/pos-pembayaran`,title:`Pos Pembayaran`,icon:`🏷️`,render:ge},{path:`/identitas-sekolah`,title:`Identitas Sekolah`,icon:`⚙️`,render:_e}]);let n=t(`#app`);if(!n)return;n.innerHTML=``,n.classList.add(`app-shell`),n.appendChild(j()),n.appendChild(M()),n.appendChild(te());let r=document.createElement(`main`);r.className=`main-content`,r.id=`main-content`,n.appendChild(r),e.onRouteChange(t=>{let a=k.getRole();if(!a){if(t!==`/login`){e.navigate(`/login`);return}}else if(a===`siswa`){if(t!==`/portal-siswa`){i(`Akses dibatasi. Siswa hanya dapat mengakses Portal Pembayaran mandiri.`,`warning`),e.navigate(`/portal-siswa`);return}}else if(a===`admin`&&t===`/login`){e.navigate(`/`);return}let o=t===`/login`;n.classList.toggle(`login-mode`,o),window.dispatchEvent(new CustomEvent(`app:auth-changed`));let s=e.getCurrentRoute();if(s){ne(t);let e=E.getSchoolInfo();if(document.title=`${s.title} — ${e.namaSekolah}`,r.innerHTML=``,!o&&a===`admin`&&t===`/`&&!D.useApi){let e=document.createElement(`div`);e.className=`config-banner animate-fade-in-down`,e.innerHTML=`
        ...
        `,r.appendChild(e)}r.appendChild(s.render())}}),e.start(),O.requestPermission(),setTimeout(()=>{O.checkAndNotify()},3e3)}document.addEventListener(`DOMContentLoaded`,ye);