(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=new class{routes=new Map;listeners=[];currentPath=``;register(e){this.routes.set(e.path,e)}registerAll(e){e.forEach(e=>this.register(e))}navigate(e){window.location.hash=`#${e}`}getCurrentPath(){return window.location.hash.slice(1)||`/`}getCurrentRoute(){return this.routes.get(this.getCurrentPath())}getRoutes(){return Array.from(this.routes.values())}onRouteChange(e){this.listeners.push(e)}start(){let e=()=>{let e=this.getCurrentPath();e!==this.currentPath&&(this.currentPath=e,this.listeners.forEach(t=>t(e)))};window.addEventListener(`hashchange`,e),window.location.hash?e():this.navigate(`/`)}};function t(e,t=document){return t.querySelector(e)}function n(e,t,...n){let r=document.createElement(e);if(t)for(let[e,n]of Object.entries(t))e===`className`?r.className=n:e===`innerHTML`?r.innerHTML=n:e===`textContent`?r.textContent=n:r.setAttribute(e,n);for(let e of n)typeof e==`string`?r.appendChild(document.createTextNode(e)):r.appendChild(e);return r}var r={success:`✅`,error:`❌`,warning:`⚠️`,info:`ℹ️`};function i(e,i=`info`,o=4e3){let s=t(`.toast-container`);s||(s=n(`div`,{className:`toast-container`}),document.body.appendChild(s));let c=n(`div`,{className:`toast toast-${i} toast-enter`,innerHTML:`
    ...
    `});typeof r!=`string`&&a.querySelector(`.modal-body`).appendChild(r),i.appendChild(a),document.body.appendChild(i),requestAnimationFrame(()=>i.classList.add(`visible`));let o=()=>{i.classList.remove(`visible`),setTimeout(()=>i.remove(),300)};i.querySelector(`.modal-close`)?.addEventListener(`click`,o),i.addEventListener(`click`,e=>{e.target===i&&o()});let s=e=>{e.key===`Escape`&&(o(),document.removeEventListener(`keydown`,s))};return document.addEventListener(`keydown`,s),a}function s(){let e=t(`.modal-backdrop`);e&&(e.classList.remove(`visible`),setTimeout(()=>e.remove(),300))}function c(e){return new Promise(t=>{let r=o(`Konfirmasi`,n(`div`,{innerHTML:`
      ...
      `}));r.querySelector(`#confirm-ok`)?.addEventListener(`click`,()=>{s(),t(!0)}),r.querySelector(`#confirm-cancel`)?.addEventListener(`click`,()=>{s(),t(!1)})})}var l={namaSekolah:`SMP Negeri 1 Nusantara`,alamatSekolah:`Jl. Pendidikan No. 1, Kota Nusantara`,tahunAjaran:`2026/2027`,nominalSppDefault:25e4,appsScriptUrl:``},u=[`Januari`,`Februari`,`Maret`,`April`,`Mei`,`Juni`,`Juli`,`Agustus`,`September`,`Oktober`,`November`,`Desember`],d=[{value:`tunai`,label:`Tunai (Kasir)`},{value:`transfer`,label:`Transfer Bank Manual`},{value:`qris`,label:`QRIS (Semua Pembayaran)`},{value:`va_bca`,label:`Virtual Account BCA`},{value:`va_bri`,label:`Virtual Account BRI`},{value:`va_mandiri`,label:`Virtual Account Mandiri`},{value:`gopay`,label:`GoPay`},{value:`dana`,label:`DANA`},{value:`ovo`,label:`OVO`}],f=[{id:`pos-spp`,nama:`SPP Bulanan`,kategori:`spp`,nominalDefault:25e4,deskripsi:`Iuran Pembinaan Pendidikan bulanan (dapat memilih bulan yang ingin dibayar)`,isMonthly:!0},{id:`pos-gedung`,nama:`Uang Gedung / DSP (Uang Pangkal)`,kategori:`gedung`,nominalDefault:5e5,deskripsi:`Dana Sumbangan Pendidikan / fasilitas sarana sekolah`},{id:`pos-seragam`,nama:`Paket Seragam & Atribut Sekolah`,kategori:`seragam`,nominalDefault:35e4,deskripsi:`3 stel seragam (OSIS, Pramuka, Batik) + bet & dasi`},{id:`pos-buku`,nama:`Paket Buku Pelajaran & Modul LKS`,kategori:`buku`,nominalDefault:15e4,deskripsi:`Paket modul semester ganjil/genap lengkap`},{id:`pos-ujian`,nama:`Biaya Ujian & Asesmen Semester (PTS/PAS)`,kategori:`ujian`,nominalDefault:1e5,deskripsi:`Operasional ujian berbasis komputer (CBT) & administrasi`},{id:`pos-kegiatan`,nama:`Kegiatan Outing & Ekstrakurikuler`,kategori:`kegiatan`,nominalDefault:75e3,deskripsi:`Kegiatan luar kelas, kepramukaan, dan pengembangan minat`},{id:`pos-bebas`,nama:`Tagihan Lainnya / Donasi Sukarela / Cicilan`,kategori:`bebas`,nominalDefault:5e4,isCustomNominal:!0,deskripsi:`Nominal bebas! Anda dapat membayar berapapun sesuai kemampuan atau jenis keperluan khusus.`}],p={STUDENTS:`spp_students`,PAYMENTS:`spp_payments`,CONFIG:`spp_config`,THEME:`spp_theme`,NOTIFICATIONS:`spp_notifications`,SCHOOL_INFO:`spp_school_info`,BILLABLE_ITEMS:`spp_billable_items`,AUTH_SESSION:`spp_auth_session`,ADMIN_PASSWORD:`spp_admin_password`};function m(e){return new Intl.NumberFormat(`id-ID`,{style:`currency`,currency:`IDR`,minimumFractionDigits:0,maximumFractionDigits:0}).format(e)}function h(e){let t=new Date(e);return new Intl.DateTimeFormat(`id-ID`,{day:`numeric`,month:`long`,year:`numeric`}).format(t)}function g(e){let t=new Date(e);return new Intl.DateTimeFormat(`id-ID`,{day:`2-digit`,month:`2-digit`,year:`numeric`}).format(t)}function _(e=new Date){return e.toISOString().split(`T`)[0]}function v(){let e=new Date;return`SPP-${e.getFullYear().toString().slice(-2)}${String(e.getMonth()+1).padStart(2,`0`)}${String(e.getDate()).padStart(2,`0`)}-${Math.random().toString(36).substring(2,6).toUpperCase()}`}function y(e){return{lunas:`Lunas`,belum:`Belum Bayar`,sebagian:`Sebagian`}[e]??e}function b(e){return{lunas:`badge-success`,belum:`badge-danger`,sebagian:`badge-warning`}[e]??`badge-info`}function x(e){return{tunai:`Tunai`,transfer:`Transfer Bank`,qris:`QRIS`,va_bca:`VA BCA`,va_bri:`BRIVA (BRI)`,va_mandiri:`VA Mandiri`,gopay:`GoPay`,dana:`DANA`,ovo:`OVO`}[e]??e}function S(){return new Date().getMonth()}function C(){return new Date().getFullYear()}var w={namaSekolah:l.namaSekolah,alamatSekolah:l.alamatSekolah,noTelepon:`(021) 789-0123`,email:`info@smpn1nusantara.sch.id`,tahunAjaran:l.tahunAjaran,nominalSppDefault:l.nominalSppDefault,namaKepalaSekolah:`Dr. H. Bambang Sudiro, M.Pd.`,nipKepalaSekolah:`19750812 200003 1 002`,namaBendahara:`Siti Rahmawati, S.E.`,nipBendahara:`19820415 200801 2 007`,catatanKuitansi:`Kuitansi ini adalah bukti pembayaran yang sah dan tersimpan secara elektronik di database sekolah.`,appsScriptUrl:l.appsScriptUrl||``},T=new class{getSchoolInfo(){try{let e=localStorage.getItem(p.SCHOOL_INFO);if(e)return{...w,...JSON.parse(e)}}catch{}return{...w}}updateSchoolInfo(e){let t={...this.getSchoolInfo(),...e};try{localStorage.setItem(p.SCHOOL_INFO,JSON.stringify(t)),l.namaSekolah=t.namaSekolah,l.alamatSekolah=t.alamatSekolah,l.tahunAjaran=t.tahunAjaran,l.nominalSppDefault=t.nominalSppDefault,t.appsScriptUrl!==void 0&&(l.appsScriptUrl=t.appsScriptUrl)}catch(e){console.warn(`Gagal menyimpan identitas sekolah ke storage:`,e)}return window.dispatchEvent(new CustomEvent(`app:school-info-updated`,{detail:t})),t}getBillableItems(){try{let e=localStorage.getItem(p.BILLABLE_ITEMS);if(e){let t=JSON.parse(e);if(Array.isArray(t)&&t.length>0)return t}}catch{}return this.saveBillableItems([...f]),[...f]}saveBillableItems(e){try{localStorage.setItem(p.BILLABLE_ITEMS,JSON.stringify(e))}catch(e){console.warn(`Gagal menyimpan pos pembayaran:`,e)}window.dispatchEvent(new CustomEvent(`app:billable-items-updated`,{detail:e}))}addBillableItem(e){let t=this.getBillableItems(),n={id:`pos-`+Date.now()+`-`+Math.random().toString(36).substring(2,6),...e};return t.push(n),this.saveBillableItems(t),n}updateBillableItem(e,t){let n=this.getBillableItems(),r=n.findIndex(t=>t.id===e);return r!==-1&&(n[r]={...n[r],...t},this.saveBillableItems(n),!0)}deleteBillableItem(e){let t=this.getBillableItems(),n=t.filter(t=>t.id!==e);return n.length!==t.length&&(this.saveBillableItems(n),!0)}resetBillableItems(){this.saveBillableItems([...f])}},E=new class{get apiUrl(){return T.getSchoolInfo().appsScriptUrl||l.appsScriptUrl||``}get useApi(){return!!this.apiUrl}async testConnection(e){let t=e||this.apiUrl;if(!t)return{success:!1,message:`URL Google Apps Script belum diisi.`};try{let e=new URL(t);e.searchParams.set(`action`,`getStudents`);let n=await(await fetch(e.toString())).json();return n&&n.success!==void 0?{success:!0,message:`Koneksi ke Google Spreadsheet BERHASIL & AKTIF!`}:{success:!1,message:`Respons API tidak valid. Pastikan Who has access diatur Anyone.`}}catch(e){return{success:!1,message:`Gagal menghubungi URL: `+String(e)}}}async apiGet(e,t){if(!this.useApi)throw Error(`API URL not configured`);let n=new URL(this.apiUrl);n.searchParams.set(`action`,e),t&&Object.entries(t).forEach(([e,t])=>n.searchParams.set(e,t));try{return await(await fetch(n.toString())).json()}catch(t){return console.error(`API GET error (${e}):`,t),{success:!1,error:String(t)}}}async apiPost(e,t){if(!this.useApi)throw Error(`API URL not configured`);try{return await(await fetch(this.apiUrl,{method:`POST`,headers:{"Content-Type":`text/plain`},body:JSON.stringify({action:e,...t})})).json()}catch(t){return console.error(`API POST error (${e}):`,t),{success:!1,error:String(t)}}}getLocal(e){try{let t=localStorage.getItem(e);return t?JSON.parse(t):[]}catch{return[]}}setLocal(e,t){localStorage.setItem(e,JSON.stringify(t))}async getStudents(){return this.useApi?(await this.apiGet(`getStudents`)).data??[]:this.getLocal(p.STUDENTS)}async addStudent(e){if(this.useApi)return(await this.apiPost(`addStudent`,{student:e})).success;let t=this.getLocal(p.STUDENTS);return!t.some(t=>t.nis===e.nis)&&(t.push(e),this.setLocal(p.STUDENTS,t),!0)}async importStudents(e,t=!0){let n=await this.getStudents(),r=new Map;n.forEach(e=>r.set(e.nis.trim(),e));let i=0,a=0,o=0;for(let n of e){let e=n.nis.trim();e&&(r.has(e)?t?(r.set(e,{...r.get(e),...n}),a++):o++:(r.set(e,n),i++))}let s=Array.from(r.values());if(this.setLocal(p.STUDENTS,s),this.useApi)try{await this.apiPost(`bulkImportStudents`,{students:s})}catch(e){console.warn(`API sync bulkImportStudents failed, saved locally:`,e)}return{added:i,updated:a,skipped:o}}async updateStudent(e,t){if(this.useApi)return(await this.apiPost(`updateStudent`,{nis:e,student:t})).success;let n=this.getLocal(p.STUDENTS),r=n.findIndex(t=>t.nis===e);return r!==-1&&(n[r]={...n[r],...t},this.setLocal(p.STUDENTS,n),!0)}async deleteStudent(e){if(this.useApi)return(await this.apiPost(`deleteStudent`,{nis:e})).success;let t=this.getLocal(p.STUDENTS),n=t.filter(t=>t.nis!==e);return n.length!==t.length&&(this.setLocal(p.STUDENTS,n),!0)}async getStudentByNis(e){return(await this.getStudents()).find(t=>t.nis.trim().toLowerCase()===e.trim().toLowerCase())}async getPayments(){return this.useApi?(await this.apiGet(`getPayments`)).data??[]:this.getLocal(p.PAYMENTS)}async addPayment(e){if(!e.rincianItemText&&e.items&&e.items.length>0?e.rincianItemText=e.items.map(e=>`${e.nama} (${m(e.nominal)})`).join(`, `):e.rincianItemText||=`SPP ${e.bulan} ${e.tahun}`,e.channel||=`admin`,this.useApi)return(await this.apiPost(`addPayment`,{payment:e})).success;let t=this.getLocal(p.PAYMENTS);return(!e.items||e.items.length<=1)&&t.some(t=>t.nis===e.nis&&t.bulan===e.bulan&&t.tahun===e.tahun&&t.status===`lunas`&&(!t.items||t.items.some(e=>e.kategori===`spp`)))?!1:(t.push(e),this.setLocal(p.PAYMENTS,t),!0)}async getStudentPayments(e,t){return(await this.getPayments()).filter(n=>n.nis===e&&(!t||n.tahun===t))}async deletePayment(e){if(this.useApi)return(await this.apiPost(`deletePayment`,{idTransaksi:e})).success;let t=this.getLocal(p.PAYMENTS),n=t.filter(t=>t.idTransaksi!==e);return n.length!==t.length&&(this.setLocal(p.PAYMENTS,n),!0)}async getPaymentById(e){return(await this.getPayments()).find(t=>t.idTransaksi===e)}async getDashboardStats(e,t){let n=await this.getStudents(),r=await this.getPayments(),i=r.filter(n=>n.bulan===e&&n.tahun===t&&n.status===`lunas`||n.items&&n.items.some(n=>n.bulan===e&&n.tahun===t)),a=new Set(i.map(e=>e.nis)),o=n.filter(e=>a.has(e.nis)).length,s=Math.max(0,n.length-o),c=r.filter(e=>e.status===`lunas`).reduce((e,t)=>e+t.nominal,0),u=r.filter(e=>e.channel===`online`&&e.status===`lunas`).reduce((e,t)=>e+t.nominal,0),d=s*l.nominalSppDefault;return{totalSiswa:n.length,sudahBayar:o,belumBayar:s,totalPemasukan:c,totalTunggakan:d,totalOnline:u}}async getUnpaidStudents(e,t){let n=await this.getStudents(),r=await this.getPayments(),i=new Set(r.filter(n=>n.bulan===e&&n.tahun===t&&n.status===`lunas`||n.items&&n.items.some(n=>n.bulan===e&&n.tahun===t)).map(e=>e.nis));return n.filter(e=>!i.has(e.nis))}},D=new class{permission=`default`;audioCtx=null;async requestPermission(){if(!(`Notification`in window))return console.warn(`Browser does not support notifications`),!1;if(Notification.permission===`granted`)return this.permission=`granted`,!0;if(Notification.permission!==`denied`){let e=await Notification.requestPermission();return this.permission=e,e===`granted`}return!1}get isEnabled(){return this.permission===`granted`||Notification?.permission===`granted`}playNotificationSound(){try{let e=window.AudioContext||window.webkitAudioContext;if(!e)return;this.audioCtx||=new e,this.audioCtx.state===`suspended`&&this.audioCtx.resume();let t=this.audioCtx.currentTime,n=this.audioCtx.createOscillator(),r=this.audioCtx.createGain();n.type=`sine`,n.frequency.setValueAtTime(880,t),r.gain.setValueAtTime(.15,t),r.gain.exponentialRampToValueAtTime(.001,t+.4),n.connect(r),r.connect(this.audioCtx.destination),n.start(t),n.stop(t+.4);let i=this.audioCtx.createOscillator(),a=this.audioCtx.createGain();i.type=`sine`,i.frequency.setValueAtTime(1174.66,t+.12),a.gain.setValueAtTime(.2,t+.12),a.gain.exponentialRampToValueAtTime(.001,t+.6),i.connect(a),a.connect(this.audioCtx.destination),i.start(t+.12),i.stop(t+.6)}catch(e){console.warn(`Audio playback not permitted or not supported:`,e)}}send(e,t,n=`🏫`){if(!this.isEnabled){i(t,`info`);return}try{let r=new Notification(e,{body:t,icon:`data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y=".9em" font-size="90">${n}</text></svg>`,badge:`data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y=".9em" font-size="90">📋</text></svg>`,tag:`spp-notification`});r.onclick=()=>{window.focus(),r.close()},setTimeout(()=>r.close(),8e3)}catch{i(t,`info`)}}sendReminder(e){let t=u[S()];this.send(`⚠️ Pengingat SPP - ${l.namaSekolah}`,`${e.nama} (${e.kelas}) belum membayar SPP bulan ${t}. Nominal: ${m(e.nominalSpp)}`,`💰`)}async checkAndNotify(){let e=u[S()],t=new Date().getFullYear();try{let n=await E.getUnpaidStudents(e,t);n.length>0&&(this.send(`📋 Tunggakan SPP - ${e} ${t}`,`${n.length} siswa belum membayar SPP bulan ${e} ${t}.`,`⚠️`),i(`${n.length} siswa belum membayar SPP bulan ${e}`,`warning`,6e3))}catch(e){console.error(`Error checking notifications:`,e)}}sendPaymentSuccess(e,t,n){this.playNotificationSound(),this.send(`✅ Pembayaran Berhasil`,`${e} telah membayar SPP bulan ${t} sebesar ${m(n)}.`,`✅`)}notifyNewOnlinePayment(e){this.playNotificationSound();let t=`🔔 Pembayaran Online Diterima!`,n=e.rincianItemText||`Pembayaran ${e.bulan} ${e.tahun}`,r=`${e.nama} (${e.kelas}) membayar ${m(e.nominal)} untuk [${n}].`;this.send(t,r,`💳`),i(`🎉 ${r}`,`success`,7e3);let a=this.getNotifications(),o={id:`notif-`+Date.now()+`-`+Math.random().toString(36).substring(2,6),timestamp:Date.now(),title:t,message:r,idTransaksi:e.idTransaksi,nominal:e.nominal,namaSiswa:e.nama,read:!1};a.unshift(o),a.length>50&&(a.length=50);try{localStorage.setItem(p.NOTIFICATIONS,JSON.stringify(a))}catch(e){console.warn(`Failed saving notifications to storage:`,e)}window.dispatchEvent(new CustomEvent(`app:new-notification`,{detail:o}))}getNotifications(){try{let e=localStorage.getItem(p.NOTIFICATIONS);return e?JSON.parse(e):[]}catch{return[]}}getUnreadCount(){return this.getNotifications().filter(e=>!e.read).length}markAllAsRead(){let e=this.getNotifications();e.forEach(e=>e.read=!0);try{localStorage.setItem(p.NOTIFICATIONS,JSON.stringify(e))}catch{}window.dispatchEvent(new CustomEvent(`app:notifications-read`))}clearNotifications(){localStorage.removeItem(p.NOTIFICATIONS),window.dispatchEvent(new CustomEvent(`app:notifications-read`))}},O=`admin123`,k=new class{getSession(){try{let e=localStorage.getItem(p.AUTH_SESSION);if(e)return JSON.parse(e)}catch{}return null}getRole(){let e=this.getSession();return e?e.role:null}isAdmin(){return this.getRole()===`admin`}isStudent(){return this.getRole()===`siswa`}getCurrentStudent(){return this.getSession()?.student}getAdminPassword(){return localStorage.getItem(p.ADMIN_PASSWORD)||O}loginAsAdmin(e,t){let n=e.trim().toLowerCase()===`admin`,r=t===this.getAdminPassword();if(!n||!r)return{success:!1,error:`Username atau Password Admin salah! (Default: admin / admin123)`};let i={role:`admin`,username:`admin`,loginTime:Date.now()};return localStorage.setItem(p.AUTH_SESSION,JSON.stringify(i)),window.dispatchEvent(new CustomEvent(`app:auth-changed`,{detail:i})),{success:!0}}async loginAsStudent(e){let t=e.trim().toLowerCase();if(!t)return{success:!1,error:`Masukkan Nama atau NIS siswa!`};let n=(await E.getStudents()).find(e=>e.nis.toLowerCase()===t||e.nama.toLowerCase().includes(t));if(!n)return{success:!1,error:`Siswa dengan Nama atau NIS "${e}" tidak ditemukan di database sekolah.`};let r={role:`siswa`,student:n,loginTime:Date.now()};return localStorage.setItem(p.AUTH_SESSION,JSON.stringify(r)),window.dispatchEvent(new CustomEvent(`app:auth-changed`,{detail:r})),{success:!0,student:n}}logout(){localStorage.removeItem(p.AUTH_SESSION),window.dispatchEvent(new CustomEvent(`app:auth-changed`,{detail:null}))}updateAdminPassword(e,t){return e===this.getAdminPassword()?t.length<4?{success:!1,error:`Password baru minimal 4 karakter!`}:(localStorage.setItem(p.ADMIN_PASSWORD,t),{success:!0}):{success:!1,error:`Password lama salah!`}}};function ee(){let t=n(`aside`,{className:`sidebar`,id:`sidebar`});function r(){let r=k.getRole(),a=T.getSchoolInfo(),o=k.getCurrentStudent();if((window.location.hash.slice(1)||`/`)===`/login`||!r){t.style.display=`none`;return}if(t.style.display=`flex`,r===`siswa`&&o){t.innerHTML=`
      ...
      `,t.querySelector(`#btn-sidebar-logout`)?.addEventListener(`click`,()=>{k.logout(),i(`Anda telah keluar dari akun siswa`,`info`),e.navigate(`/login`)});return}t.innerHTML=`
      <div class="sidebar-header">
        <div class="sidebar-brand">
          <div class="sidebar-logo">🏫</div>
          <div>
    ...
    `;let s=t.querySelector(`#sidebar-nav`);[{path:`/`,title:`Dashboard`,icon:`🏠`},{path:`/siswa`,title:`Data Siswa`,icon:`👨‍🎓`},{path:`/pembayaran`,title:`Kasir Pembayaran`,icon:`💰`},{path:`/riwayat`,title:`Riwayat Pembayaran`,icon:`📋`},{path:`/pos-pembayaran`,title:`Pos Pembayaran`,icon:`🏷️`},{path:`/identitas-sekolah`,title:`Identitas Sekolah`,icon:`⚙️`},{path:`/portal-siswa`,title:`Pratinjau Portal Siswa`,icon:`💳`}].forEach(t=>{let r=t.path===`/portal-siswa`,i=n(`a`,{className:`nav-link ${r?`nav-link-portal`:``}`,"data-path":t.path,innerHTML:`
        ...
        `});i.addEventListener(`click`,n=>{n.preventDefault(),e.navigate(t.path),ne()}),s.appendChild(i)});let c=t.querySelector(`#sidebar-bell-btn`),l=t.querySelector(`#bell-badge-count`),u=()=>{let e=D.getUnreadCount();e>0?(l.textContent=e>99?`99+`:String(e),l.style.display=`flex`,c.classList.add(`has-unread`)):(l.style.display=`none`,c.classList.remove(`has-unread`))};u(),c.addEventListener(`click`,()=>{A(u)}),t.querySelector(`#btn-sidebar-logout`)?.addEventListener(`click`,()=>{k.logout(),i(`Anda telah logout dari Panel Admin`,`info`),e.navigate(`/login`)})}return r(),window.addEventListener(`app:auth-changed`,()=>{r(),j(window.location.hash.slice(1)||`/`)}),window.addEventListener(`app:school-info-updated`,()=>{r()}),window.addEventListener(`app:new-notification`,()=>{let e=t.querySelector(`#bell-badge-count`),n=t.querySelector(`#sidebar-bell-btn`);if(e&&n){let t=D.getUnreadCount();t>0&&(e.textContent=t>99?`99+`:String(t),e.style.display=`flex`,n.classList.add(`has-unread`))}}),t}function A(e){let t=D.getNotifications(),n=document.createElement(`div`);n.innerHTML=`
      ...
      `:t.map(e=>`
        <div class="notification-item ${e.read?`read`:`unread`}">
          ...
          <div class="notif-content">
            ...
            <div class="notif-time">${g(new Date(e.timestamp).toISOString())} • ${new Date(e.timestamp).toLocaleTimeString(`id-ID`,{hour:`2-digit`,minute:`2-digit`})}</div>
          </div>
        </div>
      `).join(``)}
    </div>
  `,n.querySelector(`#btn-mark-all-read`)?.addEventListener(`click`,()=>{D.markAllAsRead(),e(),A(e)}),n.querySelector(`#btn-clear-all-notifs`)?.addEventListener(`click`,()=>{D.clearNotifications(),e(),A(e)}),o(`🔔 Pusat Notifikasi Admin`,n)}function j(e){document.querySelectorAll(`.nav-link`).forEach(t=>{let n=t.getAttribute(`data-path`);t.classList.toggle(`active`,n===e)})}function te(){let e=n(`button`,{className:`mobile-menu-btn`,id:`mobile-menu-btn`,innerHTML:`☰`});return e.addEventListener(`click`,N),e}function M(){let e=n(`div`,{className:`sidebar-overlay`,id:`sidebar-overlay`});return e.addEventListener(`click`,ne),e}function N(){let e=t(`#sidebar`),n=t(`#sidebar-overlay`);e?.classList.toggle(`open`),n?.classList.toggle(`visible`)}function ne(){let e=t(`#sidebar`),n=t(`#sidebar-overlay`);e?.classList.remove(`open`),n?.classList.remove(`visible`)}function P(){let e=n(`div`,{className:`page-enter`});return e.innerHTML=`
    <div class="page-header">
      <h1 class="page-title">Dashboard</h1>
      <p class="page-description">Ringkasan pembayaran SPP & tagihan sekolah bulan ${u[S()]} ${C()}</p>
    </div>
  ...
  `}async function I(e){let t=u[S()],n=C();try{let[r,i,a]=await Promise.all([E.getDashboardStats(t,n),E.getUnpaidStudents(t,n),E.getPayments()]),o=a.filter(e=>e.channel===`online`),s=o.reduce((e,t)=>e+t.nominal,0),c=e.querySelector(`#stats-container`);c&&(c.innerHTML=``,c.classList.add(`stagger-children`),c.appendChild(L(`👨‍🎓`,`Total Siswa`,r.totalSiswa.toString(),`Terdaftar aktif`,`primary`)),c.appendChild(L(`✅`,`Sudah Bayar SPP`,r.sudahBayar.toString(),`Bulan ${t}`,`success`)),c.appendChild(L(`⚠️`,`Belum Bayar SPP`,r.belumBayar.toString(),`Bulan ${t}`,`warning`)),c.appendChild(L(`💳`,`Setoran Online`,m(s),`${o.length} transaksi online`,`primary`)),c.appendChild(L(`💰`,`Total Pemasukan`,m(r.totalPemasukan),`Semua pos tagihan`,`info`)));let l=e.querySelector(`#unpaid-list`);l&&(l.classList.remove(`animate-pulse`),i.length===0?l.innerHTML=`
          <div class="empty-state" style="padding: var(--space-8) var(--space-4);">
            <div class="empty-state-icon">🎉</div>
        ...
        `:e.map(e=>{let t=e.channel===`online`,n=e.rincianItemText||`SPP ${e.bulan} ${e.tahun}`,r=n.length>30?n.substring(0,30)+`...`:n;return`
            <div style="display: flex; align-items: center; justify-content: space-between; padding: var(--space-3) 0; border-bottom: 1px solid var(--color-border);">
              ...
              <div style="text-align: right;">
                <div style="font-weight: var(--font-weight-semibold); font-size: var(--font-size-sm); color: var(--color-success);">${m(e.nominal)}</div>
                <span class="badge badge-success" style="font-size: 10px;"><span class="badge-dot"></span> Lunas</span>
              </div>
            </div>
          `}).join(``)}}catch(e){console.error(`Dashboard data error:`,e)}}function L(e,t,r,i,a){return n(`div`,{className:`stat-card ${a}`,innerHTML:`
    ...
    `})}function R(e,t){let n=new Blob([`﻿`+t],{type:`text/csv;charset=utf-8;`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.setAttribute(`href`,r),i.setAttribute(`download`,e.endsWith(`.csv`)?e:`${e}.csv`),document.body.appendChild(i),i.click(),document.body.removeChild(i),URL.revokeObjectURL(r)}function z(e){return e==null?`""`:`"${String(e).replace(/"/g,`""`)}"`}function re(e,t=`SMP Nusantara Unggul`){let n=[`No`,`NIS`,`Nama Siswa`,`Kelas`,`Nama Orang Tua / Wali`,`No. WhatsApp / HP`,`Nominal SPP (Rp)`],r=e.map((e,t)=>[t+1,e.nis,e.nama,e.kelas,e.namaOrangTua||`-`,e.noHp||`-`,e.nominalSpp]),i=[[`Data Siswa - ${t}`],[`Diunduh pada: ${new Date().toLocaleString(`id-ID`)}`],[],n.map(z).join(`,`),...r.map(e=>e.map(z).join(`,`))].map(e=>Array.isArray(e)?e.join(`,`):e).join(`\r
`);R(`Data_Siswa_${new Date().toISOString().slice(0,10)}.csv`,i)}function B(e,t=`SMP Nusantara Unggul`){let n=[`No`,`ID Transaksi`,`Tanggal Bayar`,`NIS`,`Nama Siswa`,`Kelas`,`Rincian Pos / Item Dibayar`,`Nominal (Rp)`,`Channel`,`Metode Bayar`,`Status`,`Catatan / Keterangan`],r=e.map((e,t)=>[t+1,e.idTransaksi,h(e.tanggalBayar),e.nis,e.nama,e.kelas,e.rincianItemText||`SPP ${e.bulan} ${e.tahun}`,e.nominal,e.channel===`online`?`Online (Siswa)`:`Kasir Admin`,e.metodeBayar.toUpperCase(),e.status.toUpperCase(),e.keterangan||`-`]),i=e.reduce((e,t)=>e+(t.nominal||0),0),a=[[`Laporan Rekap Transaksi Pembayaran - ${t}`],[`Tanggal Cetak: ${new Date().toLocaleString(`id-ID`)}`],[`Total Transaksi: ${e.length} | Total Pemasukan: ${m(i)}`],[],n.map(z).join(`,`),...r.map(e=>e.map(z).join(`,`)),[],[``,``,``,``,``,`TOTAL DITERIMA`,``,i,``,``,``,``].map(z).join(`,`)].map(e=>Array.isArray(e)?e.join(`,`):e).join(`\r
`);R(`Laporan_Pembayaran_${new Date().toISOString().slice(0,10)}.csv`,a)}function V(e=25e4,t=`SMP Nusantara Unggul`){let n=`
...
*${n.namaSekolah}*`}function oe(e,t,n,r=``){let i=T.getSchoolInfo(),a=e.namaOrangTua?`Bpk/Ibu ${e.namaOrangTua}`:`Bpk/Ibu Wali`,o=t.length>0?t.join(`, `):`SPP Berjalan`;return`*PEMBERITAHUAN ADMINISTRASI SPP*
...
*${i.namaSekolah}*
📞 ${i.noTelepon||`-`}`}var J=[];function se(){let e=n(`div`,{className:`page-enter`});e.innerHTML=`
  ...
  `;let t=e.querySelector(`#student-search`),r=e.querySelector(`#student-filter-kelas`),a=e.querySelector(`#btn-add-student`);return e.querySelector(`#btn-export-students`)?.addEventListener(`click`,()=>{if(J.length===0){i(`Tidak ada data siswa untuk diexport`,`warning`);return}let e=T.getSchoolInfo();re(J,e.namaSekolah),i(`Berhasil mengekspor ${J.length} siswa ke file Excel/CSV!`,`success`)}),e.querySelector(`#btn-import-students`)?.addEventListener(`click`,()=>le(e)),t.addEventListener(`input`,()=>Y(e,t.value,r.value)),r.addEventListener(`change`,()=>Y(e,t.value,r.value)),a.addEventListener(`click`,()=>ce(e)),Y(e),e}async function Y(e,t=``,n=``){let r=e.querySelector(`#students-tbody`);if(r)try{let a=await E.getStudents(),o=e.querySelector(`#student-filter-kelas`);if(o){let e=n||o.value,t=Array.from(new Set(a.map(e=>e.kelas).filter(Boolean))).sort();o.innerHTML=`
        <option value="">Semua Kelas (${t.length} kelas)</option>
        ${t.map(t=>`<option value="${t}" ${t===e?`selected`:``}>${t}</option>`).join(``)}
      `}let s=a;if(t){let e=t.toLowerCase();s=s.filter(t=>t.nama.toLowerCase().includes(e)||t.nis.includes(e))}if(n&&(s=s.filter(e=>e.kelas===n)),J=s,s.length===0){r.innerHTML=`
        <tr>
          <td colspan="7">
      ...
      <tr>
        ...
        <td>${e.noHp}</td>
        <td style="font-weight: var(--font-weight-semibold);">${m(e.nominalSpp)}</td>
        <td>
          <div style="display: flex; gap: var(--space-2);">
    ...
    `).join(``),r.querySelectorAll(`.btn-remind-wa`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-nis`),n=s.find(e=>e.nis===t);n&&X(n)})}),r.querySelectorAll(`.btn-edit-student`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.getAttribute(`data-nis`),r=s.find(e=>e.nis===n);r&&ce(e,r)})}),r.querySelectorAll(`.btn-delete-student`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=t.getAttribute(`data-nis`),r=s.find(e=>e.nis===n);r&&await c(`Yakin ingin menghapus data siswa "${r.nama}" (${r.nis})?`)&&(await E.deleteStudent(n)?(i(`Data siswa ${r.nama} berhasil dihapus`,`success`),Y(e)):i(`Gagal menghapus data siswa`,`error`))})})}catch(e){console.error(`Error loading students:`,e),r.innerHTML=`
      <tr><td colspan="7" class="text-center text-danger" style="padding: var(--space-8);">Error memuat data siswa</td></tr>
    `}}async function X(e){let t=await E.getStudentPayments(e.nis),r=new Date().getFullYear(),i=new Date().getMonth(),a=u.slice(0,i+1),c=t.filter(e=>e.tahun===r&&e.status===`lunas`).map(e=>e.bulan),l=a.filter(e=>!c.includes(e)),d=l.length*(e.nominalSpp||15e4),f=n(`div`,{innerHTML:`
        ...
        <div style="background: var(--color-bg-glass); border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: var(--space-3); margin-bottom: var(--space-4); font-size: var(--font-size-sm);">
          ...
          <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
            ...
            <span style="color: ${l.length>0?`var(--color-danger)`:`var(--color-success)`}; font-weight: 700;">
              ${l.length>0?`${l.join(`, `)} (${m(d)})`:`Semua Lunas`}
            </span>
          </div>
        </div>
    ...
    `});o(`Pengingat SPP - ${e.nama}`,f),f.querySelector(`#btn-cancel-wa-reminder`)?.addEventListener(`click`,s),f.querySelector(`#btn-send-wa-reminder`)?.addEventListener(`click`,()=>{let t=f.querySelector(`#wa-reminder-note`).value,n=oe(e,l.length>0?l:[`Bulan Berjalan`],l.length>0?d:e.nominalSpp,t);K(e.noHp,n),s()})}async function ce(e,t){let r=!!t,a=r?`Edit Data Siswa`:`Tambah Siswa Baru`,c=await E.getStudents(),u=Array.from(new Set(c.map(e=>e.kelas).filter(Boolean))).sort(),d=n(`div`,{innerHTML:`
      <form id="student-form">
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-input" id="form-kelas" value="${t?.kelas??``}" 
    ...
    `});o(a,d),d.querySelector(`#form-cancel`)?.addEventListener(`click`,s),d.querySelector(`#student-form`).addEventListener(`submit`,async t=>{t.preventDefault();let n={nis:d.querySelector(`#form-nis`).value.trim(),nama:d.querySelector(`#form-nama`).value.trim(),kelas:d.querySelector(`#form-kelas`).value.trim(),namaOrangTua:d.querySelector(`#form-ortu`).value.trim(),noHp:d.querySelector(`#form-hp`).value.trim(),nominalSpp:Number(d.querySelector(`#form-nominal`).value)||l.nominalSppDefault};if(!n.nis||!n.nama||!n.kelas||!n.namaOrangTua){i(`Mohon lengkapi semua field yang wajib (*)`,`warning`);return}let a;a=r?await E.updateStudent(n.nis,n):await E.addStudent(n),a?(i(r?`Data ${n.nama} berhasil diperbarui`:`Siswa ${n.nama} berhasil ditambahkan`,`success`),s(),Y(e)):i(r?`Gagal memperbarui data`:`NIS sudah terdaftar atau gagal menyimpan`,`error`)})}function le(e){let t=T.getSchoolInfo(),r=[],a=n(`div`,{innerHTML:`
    ...
    `});o(`📥 Impor Data Siswa dari Excel / CSV`,a),a.querySelector(`#btn-download-excel`)?.addEventListener(`click`,()=>{V(t.nominalSppDefault,t.namaSekolah),i(`Template Excel (.xls) berhasil diunduh! Buka langsung di Microsoft Excel.`,`success`)}),a.querySelector(`#btn-download-csv`)?.addEventListener(`click`,()=>{H(t.nominalSppDefault),i(`Template CSV (.csv) berhasil diunduh!`,`success`)});let c=a.querySelector(`#import-file-input`),l=a.querySelector(`#import-dropzone`),u=a.querySelector(`#import-file-name`),d=a.querySelector(`#import-preview-section`),f=a.querySelector(`#import-preview-tbody`),p=a.querySelector(`#import-status-badge`),h=a.querySelector(`#import-errors-box`),g=a.querySelector(`#btn-confirm-import`),_=a.querySelector(`#btn-cancel-import`),v=a.querySelector(`#import-update-existing`);_.addEventListener(`click`,s),l.addEventListener(`click`,()=>c.click()),l.addEventListener(`dragover`,e=>{e.preventDefault(),l.style.borderColor=`var(--color-primary)`,l.style.background=`rgba(99, 102, 241, 0.08)`}),l.addEventListener(`dragleave`,()=>{l.style.borderColor=`var(--color-border)`,l.style.background=`rgba(255, 255, 255, 0.02)`}),l.addEventListener(`drop`,e=>{e.preventDefault(),l.style.borderColor=`var(--color-border)`,l.style.background=`rgba(255, 255, 255, 0.02)`,e.dataTransfer?.files&&e.dataTransfer.files[0]&&y(e.dataTransfer.files[0])}),c.addEventListener(`change`,()=>{c.files&&c.files[0]&&y(c.files[0])});function y(e){u.style.display=`block`,u.textContent=`📄 ${e.name} (${(e.size/1024).toFixed(1)} KB)`;let n=new FileReader;n.onload=e=>{let n=e.target?.result;if(!n){i(`File tidak memiliki konten`,`warning`);return}let a=G(n,t.nominalSppDefault);if(r=a.valid,d.style.display=`flex`,a.errors.length>0?(h.style.display=`block`,h.innerHTML=`<strong>Peringatan / Catatan (${a.errors.length}):</strong><br>`+a.errors.map(e=>`• ${e}`).join(`<br>`)):h.style.display=`none`,r.length===0){p.className=`badge badge-danger text-xs`,p.textContent=`0 Data Valid Ditemukan`,g.disabled=!0,f.innerHTML=`<tr><td colspan="6" class="text-center text-muted" style="padding: 12px;">Format tidak dikenali. Silakan periksa atau gunakan template resmi.</td></tr>`;return}p.className=`badge badge-success text-xs`,p.textContent=`✅ ${r.length} Siswa Siap Diimpor`,g.disabled=!1,g.textContent=`🚀 Impor ${r.length} Data Siswa`;let o=r.slice(0,15);f.innerHTML=o.map(e=>`
          <tr>
            ...
            <td style="padding: 5px 8px;">${m(e.nominalSpp)}</td>
          </tr>
        ...
        `)},n.readAsText(e,`UTF-8`)}g.addEventListener(`click`,async()=>{if(r.length===0)return;g.disabled=!0,g.textContent=`Menyimpan ke database...`;let t=v.checked,n=await E.importStudents(r,t);i(`Berhasil mengimpor data siswa: ${n.added} siswa baru ditambahkan, ${n.updated} diperbarui!`,`success`),s(),Y(e)})}function Z(e){let t=ue(e),n=document.createElement(`div`);n.innerHTML=`
    <div style="margin-bottom: var(--space-4);">
      ${t}
    </div>
  ...
  `,o(`Preview Kuitansi Pembayaran`,n),n.querySelector(`#btn-print-receipt`)?.addEventListener(`click`,()=>{de(e)}),n.querySelector(`#btn-wa-receipt`)?.addEventListener(`click`,async()=>{try{let t=(await E.getStudents()).find(t=>t.nis===e.nis),n=q(e,t);K(t?.noHp||``,n)}catch(t){console.error(t),K(``,q(e))}})}function ue(e){let t=T.getSchoolInfo(),n=e.items&&e.items.length>0,r=e.channel===`online`?`Online (Portal Siswa)`:`Kasir Administrasi Sekolah`;return`
    <div class="receipt" id="receipt-content">
      ...
      <div class="receipt-body">
        ...
        <div class="receipt-row">
          ...
          <span class="receipt-value">${h(e.tanggalBayar)}</span>
        </div>
        <div class="receipt-row">
        ...
        <div class="receipt-row">
          ...
          <span class="receipt-value">${x(e.metodeBayar)}</span>
        </div>
              ...
              <tbody>
                ${e.items.map(e=>`
                  <tr style="border-bottom: 1px dotted #eee;">
                    ...
                    <td style="padding: 5px 0; text-align: right; font-weight: 600;">${m(e.nominal)}</td>
                  </tr>
                `).join(``)}
              </tbody>
            </table>
        ...
        <div class="receipt-row receipt-total" style="border-top: 2px solid #111; margin-top: 10px; padding-top: 8px;">
          ...
          <span style="font-weight: 800; font-size: 15px; color: #000;">${m(e.nominal)}</span>
        </div>
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 25px; padding-top: 10px; font-size: 11px; text-align: center;">
          ...
          <div style="width: 150px;">
            <div>Dicetak Pada: ${h(e.tanggalBayar)}</div>
  ...
  `}function de(e){let t=T.getSchoolInfo(),n=window.open(``,`_blank`,`width=500,height=750`);n&&(n.document.write(`
    <!DOCTYPE html>
    <html>
    <head>
  ...
  `),n.document.close())}function fe(){let e=n(`div`,{className:`page-enter`});e.innerHTML=`
    <div class="page-header">
        ...
        <form id="payment-form">
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <select class="form-select" id="pay-tahun" required>
                ${[C()-1,C(),C()+1].map(e=>`<option value="${e}" ${e===C()?`selected`:``}>${e}</option>`).join(``)}
              </select>
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="date" class="form-input" id="pay-tanggal" value="${_()}">
            </div>
            <div class="form-group">
              <label class="form-label">Metode Bayar</label>
              <select class="form-select" id="pay-metode">
                ${d.map(e=>`<option value="${e.value}">${e.label}</option>`).join(``)}
              </select>
            </div>
          </div>
  ...
  `,Q(e);let t=e.querySelector(`#pay-student`);t.addEventListener(`change`,()=>pe(e,t.value));let r=e.querySelector(`#grid-filter-kelas`);return r.addEventListener(`change`,()=>he(e,r.value)),e.querySelector(`#payment-form`).addEventListener(`submit`,t=>ge(t,e)),e}async function Q(e){let t=e.querySelector(`#pay-student`),r=e.querySelector(`#grid-filter-kelas`),i=await E.getStudents();i.forEach(e=>{let r=n(`option`,{value:e.nis});r.textContent=`${e.nama} — ${e.kelas} (${e.nis})`,t.appendChild(r)}),r&&(r.innerHTML=`
      <option value="">Semua Kelas</option>
      ${Array.from(new Set(i.map(e=>e.kelas).filter(Boolean))).sort().map(e=>`<option value="${e}">${e}</option>`).join(``)}
    `)}async function pe(e,t){let n=e.querySelector(`#student-info-box`),r=e.querySelector(`#student-info-content`),i=e.querySelector(`#pay-nominal`),a=e.querySelector(`#payment-status-content`);if(!t){n.style.display=`none`,a.innerHTML=`<p class="text-muted" style="font-size: var(--font-size-sm);">Pilih siswa pada form untuk melihat status pembayaran.</p>`;return}let o=await E.getStudentByNis(t);o&&(n.style.display=`block`,r.innerHTML=`
    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
    ...
    <div style="display: flex; justify-content: space-between;">
      ...
      <span style="font-weight: var(--font-weight-semibold); color: var(--color-primary-light);">${m(o.nominalSpp)}</span>
    </div>
  `,i.value=String(o.nominalSpp),await me(e,o))}async function me(e,t){let n=e.querySelector(`#payment-status-content`),r=C(),i=await E.getStudentPayments(t.nis,r),a=new Set(i.filter(e=>e.status===`lunas`).map(e=>e.bulan));n.innerHTML=`
  ...
  `}async function he(e,t){let n=e.querySelector(`#payment-status-content`);if(!t){n.innerHTML=`<p class="text-muted" style="font-size: var(--font-size-sm);">Pilih kelas untuk melihat status pembayaran per kelas.</p>`;return}let r=(await E.getStudents()).filter(e=>e.kelas===t),i=await E.getPayments(),a=C();if(r.length===0){n.innerHTML=`
    ...
    `}),s+=`</div>`,n.innerHTML=s}async function ge(e,t){e.preventDefault();let n=t.querySelector(`#pay-student`).value,r=t.querySelector(`#pay-bulan`).value,a=Number(t.querySelector(`#pay-tahun`).value),o=Number(t.querySelector(`#pay-nominal`).value),s=t.querySelector(`#pay-tanggal`).value||_(),c=t.querySelector(`#pay-metode`).value,l=t.querySelector(`#pay-keterangan`).value.trim();if(!n||!r||!a||!o){i(`Mohon lengkapi semua field yang wajib`,`warning`);return}let u=await E.getStudentByNis(n);if(!u){i(`Siswa tidak ditemukan`,`error`);return}let d={idTransaksi:v(),nis:n,nama:u.nama,kelas:u.kelas,bulan:r,tahun:a,nominal:o,tanggalBayar:s,metodeBayar:c,status:`lunas`,keterangan:l};await E.addPayment(d)?(i(`Pembayaran ${u.nama} bulan ${r} berhasil dicatat!`,`success`),D.sendPaymentSuccess(u.nama,r,o),t.querySelector(`#pay-keterangan`).value=``,await pe(t,n),Z(d)):i(`${u.nama} sudah membayar SPP bulan ${r} ${a}`,`warning`)}var $=[];function _e(){let e=n(`div`,{className:`page-enter`}),t=new Date().getFullYear();e.innerHTML=`
    <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: var(--space-4);">
      <div>
  ...
  `;let r=e.querySelector(`#history-search`),a=e.querySelector(`#history-channel`),o=e.querySelector(`#history-kelas`),s=e.querySelector(`#history-bulan`),c=e.querySelector(`#history-tahun`),l=()=>{let t={search:r.value,channel:a.value,kelas:o.value,bulan:s.value,tahun:Number(c.value),status:``};ve(e,t)};return r.addEventListener(`input`,l),a.addEventListener(`change`,l),o.addEventListener(`change`,l),s.addEventListener(`change`,l),c.addEventListener(`change`,l),e.querySelector(`#btn-export-history`)?.addEventListener(`click`,()=>{if($.length===0){i(`Tidak ada data transaksi untuk diexport`,`warning`);return}let e=T.getSchoolInfo();B($,e.namaSekolah),i(`Berhasil mengekspor ${$.length} transaksi ke file Excel/CSV!`,`success`)}),l(),e}async function ve(e,t){let n=e.querySelector(`#history-tbody`),r=e.querySelector(`#history-summary`);try{let i=await E.getPayments(),a=e.querySelector(`#history-kelas`);if(a){let e=t.kelas||a.value;a.innerHTML=`
        <option value="">Semua Kelas</option>
        ${Array.from(new Set(i.map(e=>e.kelas).filter(Boolean))).sort().map(t=>`<option value="${t}" ${t===e?`selected`:``}>${t}</option>`).join(``)}
      `}let o=i;if(t.search){let e=t.search.toLowerCase();o=o.filter(t=>t.nama.toLowerCase().includes(e)||t.nis.includes(e)||t.idTransaksi.toLowerCase().includes(e)||t.rincianItemText&&t.rincianItemText.toLowerCase().includes(e))}t.channel&&(o=o.filter(e=>(e.channel||`admin`)===t.channel)),t.kelas&&(o=o.filter(e=>e.kelas===t.kelas)),t.bulan&&(o=o.filter(e=>e.bulan===t.bulan)),t.tahun&&(o=o.filter(e=>e.tahun===t.tahun)),t.status&&(o=o.filter(e=>e.status===t.status)),o.sort((e,t)=>new Date(t.tanggalBayar).getTime()-new Date(e.tanggalBayar).getTime()),$=o;let s=o.reduce((e,t)=>e+t.nominal,0),c=o.filter(e=>e.channel===`online`).length,l=o.filter(e=>e.channel===`online`).reduce((e,t)=>e+t.nominal,0);if(r.innerHTML=`
      <div style="background: var(--color-bg-glass); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: var(--space-3) var(--space-5); font-size: var(--font-size-sm);">
        ...
        <span style="font-weight: var(--font-weight-bold); margin-left: var(--space-2);">${o.length}</span>
      </div>
      <div style="background: var(--color-bg-glass); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: var(--space-3) var(--space-5); font-size: var(--font-size-sm);">
        <span style="color: var(--color-text-muted);">Total Pemasukan:</span>
        <span style="font-weight: var(--font-weight-bold); color: var(--color-success); margin-left: var(--space-2);">${m(s)}</span>
      </div>
      <div style="background: var(--color-bg-glass); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: var(--space-3) var(--space-5); font-size: var(--font-size-sm);">
        <span style="color: var(--color-text-muted);">Transaksi Online:</span>
        <span style="font-weight: var(--font-weight-bold); color: var(--color-primary-light); margin-left: var(--space-2);">${c} (${m(l)})</span>
      </div>
    `,o.length===0){n.innerHTML=`
      ...
      `;return}n.innerHTML=o.map(e=>{let t=e.channel===`online`,n=e.items?e.items.length:1,r=e.rincianItemText?e.rincianItemText.length>35?e.rincianItemText.substring(0,35)+`...`:e.rincianItemText:`SPP ${e.bulan} ${e.tahun}`;return`
        <tr>
          ...
          <td style="font-weight: var(--font-weight-semibold); color: var(--color-success);">${m(e.nominal)}</td>
          <td>
            <div style="display: flex; flex-direction: column; gap: 2px;">
              ...
              <span style="font-size: 11px; color: var(--color-text-muted);">${x(e.metodeBayar)}</span>
            </div>
          </td>
          <td><span class="badge ${b(e.status)}"><span class="badge-dot"></span> ${y(e.status)}</span></td>
          <td>
            <div style="display: flex; gap: var(--space-2);">
      ...
      `}).join(``),ye(e,o,t)}catch(e){console.error(`Error loading history:`,e),n.innerHTML=`
      <tr><td colspan="9" class="text-center text-danger" style="padding: var(--space-8);">Error memuat riwayat</td></tr>
    `}}function ye(e,t,n){e.querySelectorAll(`.btn-view-items`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.getAttribute(`data-id`),r=t.find(e=>e.idTransaksi===n);r&&be(r)})}),e.querySelectorAll(`.btn-print`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.getAttribute(`data-id`),r=t.find(e=>e.idTransaksi===n);r&&Z(r)})}),e.querySelectorAll(`.btn-wa-send`).forEach(e=>{e.addEventListener(`click`,async()=>{let n=e.getAttribute(`data-id`),r=t.find(e=>e.idTransaksi===n);if(r)try{let e=(await E.getStudents()).find(e=>e.nis===r.nis),t=q(r,e);K(e?.noHp||``,t)}catch(e){console.error(e),K(``,q(r))}})}),e.querySelectorAll(`.btn-delete-payment`).forEach(r=>{r.addEventListener(`click`,async()=>{let a=r.getAttribute(`data-id`),o=t.find(e=>e.idTransaksi===a);o&&await c(`Yakin ingin menghapus pembayaran ${o.nama}?\n\nID: ${o.idTransaksi}\nNominal: ${m(o.nominal)}`)&&(await E.deletePayment(a)?(i(`Pembayaran berhasil dihapus`,`success`),ve(e,n)):i(`Gagal menghapus pembayaran`,`error`))})})}function be(e){let t=document.createElement(`div`),n=e.channel===`online`,r=e.items&&e.items.length>0;t.innerHTML=`
    <div style="margin-bottom: var(--space-4);">
      ...
      <div style="background: var(--color-bg-glass); border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: var(--space-3); margin-bottom: var(--space-4); font-size: var(--font-size-sm);">
        ...
        <div><strong>Tanggal Bayar:</strong> ${h(e.tanggalBayar)}</div>
        <div><strong>Metode:</strong> ${x(e.metodeBayar)}</div>
        ...
        <table class="data-table" style="font-size: var(--font-size-sm);">
          ...
          <tbody>
            ${r?e.items.map((e,t)=>`
              <tr>
                ...
                <td style="text-align: right; font-weight: 600;">${m(e.nominal)}</td>
              </tr>
            `).join(``):`
              <tr>
                <td>1</td>
          ...
          <tfoot>
            <tr style="border-top: 2px solid var(--color-border); font-weight: bold; background: var(--color-bg-glass);">
              ...
              <td style="text-align: right; color: var(--color-success); font-size: var(--font-size-base);">${m(e.nominal)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
  ...
  `,o(`Rincian Pembayaran — ${e.idTransaksi}`,t),t.querySelector(`#modal-print-btn`)?.addEventListener(`click`,()=>{Z(e)})}function xe(){let t=n(`div`,{className:`page-enter`}),r=T.getSchoolInfo(),a=null,o=new Map,s=new Set,c=[];t.innerHTML=`
    <!-- Hero Banner -->
        ...
        <div class="portal-layout-grid">
          ...
          <div class="portal-items-column">
            ...
            <div class="card mb-4">
              <div class="section-header">
                <div>
                  <h3 class="section-title">📅 SPP Bulanan (${C()})</h3>
  ...
  `;let l=t.querySelector(`#portal-login-card`),d=t.querySelector(`#portal-login-form`),f=t.querySelector(`#portal-input-identity`),p=t.querySelector(`#quick-student-chips`),h=t.querySelector(`#portal-session-area`),y=t.querySelector(`#session-student-name`),b=t.querySelector(`#session-student-meta`),x=t.querySelector(`#btn-switch-student`),S=t.querySelector(`#tab-btn-tagihan`),w=t.querySelector(`#tab-btn-riwayat`),O=t.querySelector(`#tab-content-tagihan`),ee=t.querySelector(`#tab-content-riwayat`),A=t.querySelector(`#count-history-badge`),j=t.querySelector(`#student-history-tbody`),te=t.querySelector(`#total-paid-summary`),M=t.querySelector(`#portal-spp-months-container`),N=t.querySelector(`#portal-other-items-container`),ne=t.querySelector(`#spp-rate-badge`),P=t.querySelector(`#check-custom-amount`),F=t.querySelector(`#custom-amount-inputs`),I=t.querySelector(`#custom-bill-nominal`),L=t.querySelector(`#custom-bill-name`),R=t.querySelector(`#summary-items-list`),z=t.querySelector(`#summary-total-amount`),re=t.querySelector(`#selected-count-badge`),B=t.querySelector(`#btn-process-online-pay`),V=t.querySelector(`#payment-gateway-modal`),H=t.querySelector(`#gateway-modal-title`),U=t.querySelector(`#gateway-modal-body`),ie=t.querySelector(`#btn-close-gateway`),W=k.getCurrentStudent();W?K(W):ae(),d.addEventListener(`submit`,async e=>{e.preventDefault();let t=f.value.trim();t&&await q(t)}),x.addEventListener(`click`,()=>{oe()}),S.addEventListener(`click`,()=>{S.className=`btn btn-primary`,w.className=`btn btn-secondary`,O.style.display=`block`,ee.style.display=`none`}),w.addEventListener(`click`,()=>{S.className=`btn btn-secondary`,w.className=`btn btn-primary`,O.style.display=`none`,ee.style.display=`block`,ce()}),t.querySelectorAll(`input[name="portal-payment-method"]`).forEach(e=>{e.addEventListener(`change`,()=>{t.querySelectorAll(`.method-option`).forEach(e=>e.classList.remove(`active`)),e.closest(`.method-option`)?.classList.add(`active`)})}),P.addEventListener(`change`,()=>{P.checked?(F.style.display=`block`,G()):(F.style.display=`none`,o.delete(`item-custom`),X())}),I.addEventListener(`input`,()=>{P.checked&&G()}),L.addEventListener(`input`,()=>{P.checked&&G()});function G(){let e=Math.max(0,Number(I.value)||0),t=L.value.trim()||`Tagihan Bebas / Donasi`;o.set(`item-custom`,{id:`item-custom`,nama:t,kategori:`bebas`,nominal:e,keterangan:`Nominal bebas ditentukan siswa`}),X()}B.addEventListener(`click`,()=>{if(a){if(o.size===0){i(`Pilih minimal satu pos tagihan yang ingin dibayar`,`warning`);return}le(t.querySelector(`input[name="portal-payment-method"]:checked`)?.value||`qris`)}}),ie.addEventListener(`click`,()=>{V.style.display=`none`});async function ae(){let e=await E.getStudents();if(e.length===0){p.innerHTML=`<span class="text-muted text-xs">Belum ada data siswa terdaftar.</span>`;return}p.innerHTML=``,e.forEach(e=>{let t=n(`button`,{className:`btn btn-ghost btn-sm`,style:`border: 1px solid var(--color-border); border-radius: var(--radius-full); padding: 4px 12px; font-size: 11px;`,innerHTML:`👨‍🎓 ${e.nama} (${e.nis})`});t.addEventListener(`click`,()=>{q(e.nis)}),p.appendChild(t)})}async function K(e){a=e,l.style.display=`none`,h.style.display=`block`,y.textContent=e.nama,b.innerHTML=`<strong>NIS:</strong> ${e.nis} • <strong>Kelas:</strong> ${e.kelas} • <strong>Wali:</strong> ${e.namaOrangTua||`-`} • <strong>HP:</strong> ${e.noHp||`-`}`,ne.textContent=`${m(e.nominalSpp)} / bln`,await J()}async function q(e){let t=await k.loginAsStudent(e);if(!t.success||!t.student){i(t.error||`Siswa tidak ditemukan`,`error`);return}await K(t.student),i(`Selamat datang, ${t.student.nama}!`,`success`)}function oe(){a=null,k.getRole()===`admin`?(h.style.display=`none`,l.style.display=`block`,f.value=``,o.clear(),X(),i(`Selesai pratinjau akun siswa`,`info`)):(k.logout(),i(`Anda telah keluar dari akun siswa`,`info`),e.navigate(`/login`))}async function J(){a&&(c=await E.getStudentPayments(a.nis),s.clear(),c.forEach(e=>{e.status===`lunas`&&(s.add(e.bulan),e.items&&e.items.forEach(e=>{e.kategori===`spp`&&e.bulan&&s.add(e.bulan)}))}),A.textContent=String(c.length),o.clear(),P.checked=!1,F.style.display=`none`,se(),Y(),X())}function se(){M.innerHTML=``;let e=a?.nominalSpp||T.getSchoolInfo().nominalSppDefault;u.forEach(t=>{let r=s.has(t),i=`spp-${t}-${C()}`,a=o.has(i),c=n(`label`,{className:`portal-month-card ${r?`paid`:``} ${a?`selected`:``}`});if(c.innerHTML=`
        ...
        <div class="month-card-nominal">${m(e)}</div>
        <div class="month-card-status text-xs">${r?`Sudah Dilunasi`:`Belum Dibayar`}</div>
      `,!r){let n=c.querySelector(`.month-checkbox`);n.addEventListener(`change`,()=>{n.checked?(o.set(i,{id:i,nama:`SPP Bulan ${t} ${C()}`,kategori:`spp`,nominal:e,bulan:t,tahun:C()}),c.classList.add(`selected`)):(o.delete(i),c.classList.remove(`selected`)),X()})}M.appendChild(c)})}function Y(){N.innerHTML=``;let e=T.getBillableItems().filter(e=>e.kategori!==`spp`&&!e.isCustomNominal);if(e.length===0){N.innerHTML=`
      ...
      `;return}e.forEach(e=>{let t=`item-${e.id}`,r=o.has(t),i=n(`label`,{className:`portal-item-row ${r?`selected`:``}`});i.innerHTML=`
        ...
        <div class="portal-item-price">${m(e.nominalDefault)}</div>
      `;let a=i.querySelector(`.portal-item-checkbox`);a.addEventListener(`change`,()=>{a.checked?(o.set(t,{id:t,nama:e.nama,kategori:e.kategori,nominal:e.nominalDefault}),i.classList.add(`selected`)):(o.delete(t),i.classList.remove(`selected`)),X()}),N.appendChild(i)})}function X(){let e=Array.from(o.values()),t=e.length,n=e.reduce((e,t)=>e+t.nominal,0);if(re.textContent=`${t} Item`,z.textContent=m(n),t===0){R.innerHTML=`
        <div class="text-center text-muted" style="padding: var(--space-8);">
          Belum ada tagihan yang dipilih.<br>
          <small>Silakan centang item di sebelah kiri.</small>
        </div>
      `,B.disabled=!0,B.innerHTML=`🔒 Pilih Tagihan Terlebih Dahulu`;return}B.disabled=!1,B.innerHTML=`🔒 Bayar Sekarang • ${m(n)}`,R.innerHTML=e.map(e=>`
      <div class="summary-item-entry animate-fade-in">
        ...
        <div class="summary-item-price">${m(e.nominal)}</div>
      </div>
    `).join(``)}function ce(){if(!a)return;let e=c.reduce((e,t)=>e+t.nominal,0);if(te.textContent=`Total Lunas: ${m(e)}`,c.length===0){j.innerHTML=`
        <tr>
          <td colspan="7">
            <div class="empty-state" style="padding: var(--space-8) var(--space-4);">
              <div class="empty-state-icon">📭</div>
      ...
      `;return}let t=[...c].sort((e,t)=>new Date(t.tanggalBayar).getTime()-new Date(e.tanggalBayar).getTime());j.innerHTML=t.map(e=>{let t=e.items?e.items.length:1,n=e.rincianItemText||`SPP ${e.bulan} ${e.tahun}`;return`
        <tr>
          ...
          <td style="font-weight: bold; color: var(--color-success);">${m(e.nominal)}</td>
      ...
      <div class="gateway-qris-container text-center">
        ...
        <div class="gateway-amount-tag">
          ...
          <div class="amount-large">${m(t)}</div>
        </div>
    ...
    `,U.querySelector(`#btn-simulate-success`)?.addEventListener(`click`,()=>{Q(`qris`,e,t,n)})}function de(e,t,n,r){let o=T.getSchoolInfo(),s=`${e===`va_bca`?`8808`:e===`va_bri`?`1288`:`8901`}${a?.nis||`2026001`}`;U.innerHTML=`
      <div class="gateway-va-container">
        ...
        <div class="gateway-amount-tag text-center mt-3">
          ...
          <div class="amount-large">${m(n)}</div>
        </div>
    ...
    `,U.querySelector(`#btn-simulate-success`)?.addEventListener(`click`,()=>{Q(e,t,n,r)})}async function Q(e,t,n,r){if(!a)return;let o=r.find(e=>e.kategori===`spp`),s=o?.bulan||u[new Date().getMonth()],c=o?.tahun||C(),l=r.map(e=>({id:e.id,nama:e.nama,kategori:e.kategori,nominal:e.nominal,bulan:e.bulan,tahun:e.tahun,keterangan:e.keterangan})),d=r.map(e=>`${e.nama} (${m(e.nominal)})`).join(` + `),f={idTransaksi:t,nis:a.nis,nama:a.nama,kelas:a.kelas,bulan:s,tahun:c,nominal:n,tanggalBayar:_(),metodeBayar:e,status:`lunas`,keterangan:`Pembayaran Online (${r.length} pos tagihan)`,channel:`online`,items:l,rincianItemText:d};if(!await E.addPayment(f)){i(`Gagal mencatat transaksi pembayaran`,`error`);return}V.style.display=`none`,D.notifyNewOnlinePayment(f),pe(f,r),await J()}function pe(e,t){let n=T.getSchoolInfo(),r=document.createElement(`div`);r.className=`payment-success-dialog animate-scale-up`,r.innerHTML=`
    ...
    `,H.textContent=`Bukti Pembayaran — ${n.namaSekolah}`,U.innerHTML=``,U.appendChild(r),V.style.display=`flex`,r.querySelector(`#btn-print-from-success`)?.addEventListener(`click`,()=>{Z(e)}),r.querySelector(`#btn-done-success`)?.addEventListener(`click`,()=>{V.style.display=`none`,w.click()})}return t}function Se(){let e=n(`div`,{className:`page-enter`});e.innerHTML=`
    <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: var(--space-4);">
      <div>
  ...
  `;let t=e.querySelector(`#billable-tbody`),r=e.querySelector(`#billable-stats`),a=e.querySelector(`#badge-total-pos`),l=e.querySelector(`#btn-add-billable`),u=e.querySelector(`#btn-reset-billable`);function d(){let e=T.getBillableItems();a.textContent=`${e.length} Pos Tagihan`;let n=e.reduce((e,t)=>e+(t.isCustomNominal?0:t.nominalDefault),0),o=e.filter(e=>e.isCustomNominal).length;if(r.innerHTML=`
      ...
      <div class="stat-card success">
        ...
        <div class="stat-value" style="font-size: var(--font-size-xl);">${m(n)}</div>
    ...
    `,e.length===0){t.innerHTML=`
      ...
      <tr>
        ...
        <td style="font-weight: bold; color: var(--color-success); font-size: var(--font-size-sm);">
          ${e.isCustomNominal?`<span class="text-muted">Nominal Bebas (Custom)</span>`:m(e.nominalDefault)}
        </td>
        <td>
    ...
    `).join(``),t.querySelectorAll(`.btn-edit-pos`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.getAttribute(`data-id`),r=e.find(e=>e.id===n);r&&f(r)})}),t.querySelectorAll(`.btn-delete-pos`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=t.getAttribute(`data-id`),r=e.find(e=>e.id===n);r&&await c(`Yakin ingin menghapus pos pembayaran "${r.nama}"?\n\nItem ini tidak akan lagi muncul di Portal Siswa.`)&&(T.deleteBillableItem(n),i(`Pos pembayaran "${r.nama}" berhasil dihapus`,`success`),d())})})}l.addEventListener(`click`,()=>{f()}),u.addEventListener(`click`,async()=>{await c(`Kembalikan seluruh daftar pos tagihan ke pengaturan bawaan sekolah?`)&&(T.resetBillableItems(),i(`Daftar pos pembayaran berhasil di-reset ke default`,`info`),d())});function f(e){let t=!!e,n=document.createElement(`div`);n.innerHTML=`
    ...
    `,n.querySelector(`#btn-cancel-modal`)?.addEventListener(`click`,()=>{s()}),n.querySelector(`#form-billable-item`)?.addEventListener(`submit`,r=>{r.preventDefault();let a=n.querySelector(`#item-nama`).value.trim(),o=n.querySelector(`#item-kategori`).value,c=Number(n.querySelector(`#item-nominal`).value)||0,l=n.querySelector(`#item-custom-check`).checked,u=n.querySelector(`#item-deskripsi`).value.trim();if(!a){i(`Nama pembayaran tidak boleh kosong`,`warning`);return}t&&e?(T.updateBillableItem(e.id,{nama:a,kategori:o,nominalDefault:c,isCustomNominal:l,deskripsi:u}),i(`Pos pembayaran "${a}" berhasil diperbarui!`,`success`)):(T.addBillableItem({nama:a,kategori:o,nominalDefault:c,isCustomNominal:l,deskripsi:u,isMonthly:o===`spp`}),i(`Pos pembayaran baru "${a}" berhasil ditambahkan!`,`success`)),s(),d()}),o(t?`Edit Pos Pembayaran`:`Tambah Pos Pembayaran Baru`,n)}return d(),e}function Ce(){let e=n(`div`,{className:`page-enter`}),t=T.getSchoolInfo();e.innerHTML=`
    <div class="page-header">
  ...
  `;let r=e.querySelector(`#form-school-settings`),a=e.querySelector(`#set-nama-sekolah`),o=e.querySelector(`#set-alamat-sekolah`),s=e.querySelector(`#set-no-telepon`),c=e.querySelector(`#set-email`),l=e.querySelector(`#set-tahun-ajaran`),u=e.querySelector(`#set-nominal-spp`),d=e.querySelector(`#set-kepala-sekolah`),f=e.querySelector(`#set-nip-kepala`),p=e.querySelector(`#set-bendahara`),h=e.querySelector(`#set-catatan-kuitansi`),g=e.querySelector(`#set-apps-script-url`),_=e.querySelector(`#btn-test-sheet`),v=e.querySelector(`#test-sheet-status`),y=e.querySelector(`#live-receipt-preview`);function b(){let e=a.value.trim()||`Nama Sekolah`,t=o.value.trim()||`Alamat Sekolah`,n=s.value.trim()||`-`,r=c.value.trim()||`-`,i=l.value.trim()||`2026/2027`,u=d.value.trim()||`Nama Kepala Sekolah`,g=f.value.trim()||`-`,_=p.value.trim()||`Nama Bendahara`,v=h.value.trim()||`Bukti bayar sah.`;y.innerHTML=`
    ...
    `}[a,o,s,c,l,d,f,p,h].forEach(e=>{e.addEventListener(`input`,b)}),b(),_.addEventListener(`click`,async()=>{let e=g.value.trim();if(!e){v.textContent=`⚠️ Masukkan URL Apps Script terlebih dahulu.`,v.className=`text-xs text-warning`;return}v.textContent=`🔄 Sedang menguji koneksi...`,v.className=`text-xs text-muted`,_.disabled=!0;let t=await E.testConnection(e);_.disabled=!1,t.success?(v.textContent=`✅ ${t.message}`,v.className=`text-xs text-success`,i(`Koneksi Google Spreadsheet Berhasil!`,`success`)):(v.textContent=`❌ ${t.message}`,v.className=`text-xs text-danger`,i(t.message,`error`,6e3))}),r.addEventListener(`submit`,e=>{e.preventDefault();let t=T.updateSchoolInfo({namaSekolah:a.value.trim(),alamatSekolah:o.value.trim(),noTelepon:s.value.trim(),email:c.value.trim(),tahunAjaran:l.value.trim(),nominalSppDefault:Number(u.value)||25e4,namaKepalaSekolah:d.value.trim(),nipKepalaSekolah:f.value.trim(),namaBendahara:p.value.trim(),catatanKuitansi:h.value.trim(),appsScriptUrl:g.value.trim()});document.title=`Identitas Sekolah — ${t.namaSekolah}`,i(`Data Identitas & Database "${t.namaSekolah}" berhasil disimpan!`,`success`)});let x=e.querySelector(`#form-admin-password`),S=e.querySelector(`#input-old-pass`),C=e.querySelector(`#input-new-pass`);return x.addEventListener(`submit`,e=>{e.preventDefault();let t=S.value,n=C.value,r=k.updateAdminPassword(t,n);if(!r.success){i(r.error||`Gagal mengubah password`,`error`);return}i(`Password Admin berhasil diperbarui!`,`success`),x.reset()}),e}function we(){let t=n(`div`,{className:`login-page-container animate-fade-in`}),r=T.getSchoolInfo();t.innerHTML=`
  ...
  `;let a=t.querySelector(`#tab-login-siswa`),o=t.querySelector(`#tab-login-admin`),s=t.querySelector(`#box-login-siswa`),c=t.querySelector(`#box-login-admin`),l=t.querySelector(`#form-login-siswa`),u=t.querySelector(`#form-login-admin`),d=t.querySelector(`#input-siswa-identity`),f=t.querySelector(`#input-admin-user`),p=t.querySelector(`#input-admin-pass`),m=t.querySelector(`#login-quick-chips`);a.addEventListener(`click`,()=>{a.classList.add(`active`),o.classList.remove(`active`),s.style.display=`block`,c.style.display=`none`,d.focus()}),o.addEventListener(`click`,()=>{o.classList.add(`active`),a.classList.remove(`active`),c.style.display=`block`,s.style.display=`none`,p.focus()}),h();async function h(){let e=await E.getStudents();if(e.length===0){m.innerHTML=`<span class="text-xs text-muted">Belum ada data siswa terdaftar.</span>`;return}m.innerHTML=``,e.forEach(e=>{let t=n(`button`,{className:`btn btn-ghost btn-sm`,style:`border: 1px solid var(--color-border); border-radius: var(--radius-full); padding: 4px 12px; font-size: 11px;`,innerHTML:`👨‍🎓 ${e.nama} (${e.nis})`});t.addEventListener(`click`,()=>{g(e.nis)}),m.appendChild(t)})}l.addEventListener(`submit`,async e=>{e.preventDefault();let t=d.value.trim();t&&await g(t)});async function g(t){let n=await k.loginAsStudent(t);if(!n.success){i(n.error||`Gagal masuk sebagai siswa`,`error`);return}i(`Selamat datang, ${n.student?.nama}!`,`success`),e.navigate(`/portal-siswa`)}return u.addEventListener(`submit`,t=>{t.preventDefault();let n=f.value.trim(),r=p.value,a=k.loginAsAdmin(n,r);if(!a.success){i(a.error||`Login admin gagal`,`error`);return}i(`Login berhasil sebagai Administrator!`,`success`),e.navigate(`/`)}),t}function Te(){e.registerAll([{path:`/login`,title:`Masuk Akun`,icon:`🔑`,render:we},{path:`/`,title:`Dashboard`,icon:`🏠`,render:P},{path:`/portal-siswa`,title:`Portal Siswa`,icon:`💳`,render:xe},{path:`/pembayaran`,title:`Kasir Pembayaran`,icon:`💰`,render:fe},{path:`/riwayat`,title:`Riwayat Pembayaran`,icon:`📋`,render:_e},{path:`/siswa`,title:`Data Siswa`,icon:`👨‍🎓`,render:se},{path:`/pos-pembayaran`,title:`Pos Pembayaran`,icon:`🏷️`,render:Se},{path:`/identitas-sekolah`,title:`Identitas Sekolah`,icon:`⚙️`,render:Ce}]);let n=t(`#app`);if(!n)return;n.innerHTML=``,n.classList.add(`app-shell`),n.appendChild(te()),n.appendChild(M()),n.appendChild(ee());let r=document.createElement(`main`);r.className=`main-content`,r.id=`main-content`,n.appendChild(r),e.onRouteChange(t=>{let a=k.getRole();if(!a){if(t!==`/login`){e.navigate(`/login`);return}}else if(a===`siswa`){if(t!==`/portal-siswa`){i(`Akses dibatasi. Siswa hanya dapat mengakses Portal Pembayaran mandiri.`,`warning`),e.navigate(`/portal-siswa`);return}}else if(a===`admin`&&t===`/login`){e.navigate(`/`);return}let o=t===`/login`;n.classList.toggle(`login-mode`,o),window.dispatchEvent(new CustomEvent(`app:auth-changed`));let s=e.getCurrentRoute();if(s){j(t);let e=T.getSchoolInfo();if(document.title=`${s.title} — ${e.namaSekolah}`,r.innerHTML=``,!o&&a===`admin`&&t===`/`&&!E.useApi){let e=document.createElement(`div`);e.className=`config-banner animate-fade-in-down`,e.innerHTML=`
        ...
        `,r.appendChild(e)}r.appendChild(s.render())}}),e.start(),D.requestPermission(),setTimeout(()=>{D.checkAndNotify()},3e3)}document.addEventListener(`DOMContentLoaded`,Te);